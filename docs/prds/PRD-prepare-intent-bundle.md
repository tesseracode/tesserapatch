...
**Byline**: writer sub-agent, rev-1 based on dispatch HEAD `2e2a67d`, WAVE_BASE `d060ff4`
...
(**Proposed**, rev-1). **This PRD and ADR-035 must be reviewed together.**
...
- [ADR-035 intent bundle publication and history](../adrs/ADR-035-intent-bundle-publication-and-history.md) — **companion, Proposed rev-1**; the transaction, the archive and the honesty limits
...
| **Transaction lock** | A **kernel, process-lifetime** advisory lock on `.tpatch/local/intent-prepare/<slug>/prepare.lock`, released by the OS when the holding process dies (§7.4). The lock **file** carries no authority; only the kernel-held lock does. |
...
| **Tombstone** | An index entry whose blob was deliberately purged (§9.7): the generation record survives, `blob` is empty and `purged` is `true`, so the index never claims bytes it no longer has. |
...
- Ready → commit the transition: `state = defined`, `last_command = "prepare"`,
  notes per §12.3.
- **Single-file publication set.** `status.json` is the only *publication-set*
  file written, through the shipped `writeFileAtomic`
  (`internal/store/store.go:871-917`). A single rename **is** atomic in the
  ordinary POSIX sense, so this path takes **no journal and no archive** —
  there is no multi-file window to protect (ADR-035 D3). It does take the
  transaction lock, so it cannot interleave with a sibling mutating `prepare`
  (§7.4).
- **It is not, however, a single-*file* write.** `SaveFeatureStatus` also
  refreshes the derived `FEATURES.md` best-effort
  (`internal/store/store.go:363-377`). §12.3.1 states that contract in full:
  `FEATURES.md` is derived, best-effort, outside T1, never journaled and never
  rolled back, and the `--manual` write set is therefore
  `{status.json} ∪ {FEATURES.md, best-effort}` (PIB-049, PIB-257 … PIB-261).
...
before its rename is overwritten (the disclosed residual race, §7.7.3, ADR-035
...
#### 7.4.1 Why rev-0's `O_EXCL` lock was withdrawn

rev-0 acquired `.tpatch/local/intent-prepare/<slug>/prepare.lock` with
`O_CREATE|O_EXCL|O_WRONLY` plus a nonce, and removed a stale lock **only** when
its nonce matched a pending journal's — the shipped `land` discipline
(`internal/cli/land_journal.go:675-698`). Three crash points made that model
unprovable:

- **CP1** (lock created, journal not yet written), **CP2** (blobs written,
  journal not yet written) and **CP8** (journal already cleared, lock not yet
  released) all leave a lock file with **no journal to match its nonce
  against**. Under rev-0's own rule — "a lock with any other nonce, or an
  unreadable lock, is left untouched and refuses the command" — the slug was
  then **permanently refused** by every mutating `prepare`, with the only
  escape being an undocumented manual `rm`.
- Every `--manual` crash landed in the same class, because that path takes the
  lock and never writes a journal at all.
- Even with a journal, nonce-plus-inode matching cannot distinguish "the owner
  died" from "the owner is alive and mid-transaction", because a file's
  existence is data, not liveness. The shipped resource-capture design says
  this in its own header: a PID-plus-rename lock protocol has ABA and staleness
  problems that a kernel lock does not
  (`internal/rescap/lock_unix.go:6-11`).

A design whose recovery story depends on a journal that a crash may have
prevented from existing is not a recovery story. rev-1 removes the concept of a
"stale lock" entirely.

#### 7.4.2 The primitive

**The lock is kernel-held and released on process death. The lock file carries
no authority whatsoever.**

| Platform | Mechanism | Contention signal | Release |
|---|---|---|---|
| `linux`, `darwin`, `freebsd`, `netbsd`, `openbsd`, `dragonfly` | `syscall.Flock(fd, LOCK_EX\|LOCK_NB)` on a descriptor opened `O_CREATE\|O_RDWR`, `0600` — the shipped `rescap.AcquireLock` sequence (`internal/rescap/lock_unix.go:63-88`) | `EWOULDBLOCK` / `EAGAIN` | closing the descriptor, including on `SIGKILL`, panic, or power loss — there is no unlock step that can fail (`internal/rescap/lock_unix.go:43-52`) |
| `windows` | `syscall.CreateFile` with `dwShareMode = 0` (deny-share) — a whole-handle exclusive open, `OPEN_ALWAYS`, `FILE_ATTRIBUTE_NORMAL\|FILE_FLAG_OPEN_REPARSE_POINT` (`syscall.CreateFile`, `syscall.OPEN_ALWAYS`, `syscall.FILE_FLAG_OPEN_REPARSE_POINT` are all present in the pinned toolchain's `syscall` package for `GOOS=windows`) | `ERROR_SHARING_VIOLATION` (32) or `ERROR_LOCK_VIOLATION` (33), compared as `syscall.Errno` constants declared in this package because the stdlib does not export them | the OS closing the handle when the process exits |
| everything else (`solaris`, `aix`, `js`, `plan9`, `wasip1`) | **none** | — | — |

- **Path**: `.tpatch/local/intent-prepare/<slug>/prepare.lock`, `0600`, **zero
  bytes**. Nothing is written into it: it carries no nonce, no PID and no
  hostname, because nothing reads it. A leftover lock file is inert data, and
  its presence is never a refusal (PIB-284, PIB-285).
- **Held by**: mutating `prepare` (`generate`, `manual`, `regenerate`,
  `abandon`) for the whole invocation, from before recovery until after journal
  clear.
- **Not held by**: `--check`, `--dry-run`, and every other tpatch command.
- **Extraction, not invention.** The unix implementation is the shipped
  `rescap` lock moved into a new shared package (`internal/proclock`) with
  `rescap.AcquireLock` becoming a thin wrapper that keeps its refusal codes and
  its statfs preflight. Resource-capture behavior must stay byte-identical
  against pre-change goldens (PIB-286, PIB-287).
- **Fail-closed on unsupported targets.** Mutating `prepare` refuses with
  exit 3, `prepare-unsupported-platform`, before opening any root or writing
  anything — mirroring `internal/rescap/lock_unsupported.go:29-32`. This is
  **narrower than the accepted read-only check**, whose ADR-034 D5 allowlist is
  `unix || windows`: `--check` keeps working on `solaris`/`aix`, and the
  mutating modes do not. §12.6 D8 enumerates that delta; PIB-288 asserts the
  refusal precedes every write and PIB-289 asserts `--check` is unaffected on
  the same build.
- **Filesystem preflight, because `flock` is not trusted everywhere.**
  `flock(2)`'s advisory-lock semantics are only guaranteed on genuinely local
  filesystems, which is why the shipped lock runs a `statfs` allowlist check on
  the nearest existing ancestor **before** creating anything
  (`internal/rescap/statfs_linux.go:78-96`,
  `internal/rescap/statfs_darwin.go:59-75`). The extracted lock keeps that
  preflight unchanged, so a repository on a filesystem outside the allowlist
  (a network mount, for example) **refuses** with exit 3,
  `lock-filesystem-unsupported`, before any byte is written. Fail-closed is the
  only coherent choice here: a lock that may not exclude is worse than no lock,
  because the whole rev-1 recovery model rests on liveness being decidable
  (PIB-392). Q13 records a future relaxation route.
- **The Windows limit, stated rather than assumed.** A deny-share
  `CreateFile` is enforced by the local filesystem driver; on a network share
  it is enforced by the SMB redirector, which honours share modes for SMB2+ but
  is not something this PRD can verify from inside the process. There is no
  `statfs` analogue applied on Windows, so the honest statement is: local
  volumes are covered by the same guarantee as unix, and a network share
  depends on the server honouring share modes. That sentence must appear in the
  shipped docs, and PIB-393 asserts it does.

#### 7.4.3 Liveness is decidable without a journal

Acquisition has exactly two outcomes, and both are decidable from the kernel
alone:

| Outcome | Meaning | Behavior |
|---|---|---|
| lock acquired | **No live holder.** Any leftover lock file, journal, staging tree or blob is inert state from a dead process. | proceed to recovery (§7.11) |
| contention errno | **A live process holds it right now.** | refuse: exit 3, `transaction-in-progress`, zero writes, remediation "another `tpatch prepare` is running for this feature in this workspace; wait for it to finish" (PIB-053, PIB-124, PIB-290) |

This is the property rev-0 lacked: **CP1, CP2, CP8 and every `--manual` crash
are now ordinary.** The next invocation acquires the lock (the dead process's
kernel lock is gone), finds whatever control state exists, and recovers or
proceeds. No crash in any phase can permanently brick a slug (PIB-291 …
PIB-295).

Note the deliberate exit-code change from rev-0: lock contention is exit **3**
(a transient cannot-act refusal), not exit 6. Exit 6 now means only "manual
intervention required" — divergent evidence or a failed undo — and every exit-6
population has the §6.6 escape route.

#### 7.4.4 What the lock excludes, and what it does not

The lock excludes another mutating `prepare` on the same slug. It does **not**
exclude `tpatch define`, `tpatch cycle`, an editor, or a script, because none
of them take it, and this PRD does not add lock acquisition to any existing
command (that would be a behavior delta on shipped verbs with its own deadlock
and compatibility surface). What protects against those writers is not
exclusion but **detection**:

> **Publish-time revalidation, then per-entry CAS.** Immediately before the
> first rename, under the lock, every publication-set entry's canonical file is
> re-inspected and its identity compared to the preimage captured at preflight
> (§7.8 step 4). Any mismatch — including an artifact that appeared where
> preflight saw `absent` — aborts the invocation **before** the window opens,
> exit 5, zero canonical mutations. Then each individual entry is CAS-checked
> again immediately before its own rename (§7.8 step 8), so an editor that
> writes *between* the set-level revalidation and a later entry's rename is
> still caught for that entry (PIB-100, PIB-101, PIB-102, PIB-296, PIB-297).

The residual window is between one entry's CAS and that entry's `rename` — a
few instructions, not a phase. A write that lands there is overwritten and its
bytes are **not** archived. This PRD states that limit rather than papering
over it (ADR-035 D6, PIB-103).
...
| rewrite `index.json` | equals the index entry's `preimage` | exit 5, `archive-index-changed`; nothing written |
...
| create a directory | `(*os.Root).Mkdir` / `MkdirAll` | `0755` tracked, `0700` local |
...
**No `os.Rename`, `os.CreateTemp`, `os.WriteFile`, `os.Remove` or
`filepath.Join(repoRoot, …)` appears in the publication path.** PIB-308
source-scans for exactly that, and PIB-309 is its sensitivity fixture.

#### 7.7.2 What this closes, and what it does not

- **Closed: ancestor pathname escape.** `*os.Root` resolves every component
  relative to the held directory handle and refuses any name that escapes the
  root, so no concurrent rename of `features/` or `<slug>/` can redirect a
  publication write outside the tree (PIB-310).
- **Closed: writing *through* a symlink at the destination.** `rename(2)`
  replaces a final-component symlink rather than following it, and §13.2's
  pre-rename `Lstat` gate refuses the publication anyway, so an operator's
  deliberate symlink is neither followed nor silently destroyed (PIB-148).
- **Not closed: content identity.** Between an entry's CAS and its rename, a
  concurrent writer can still replace the destination's bytes; the rename then
  overwrites them. That is the residual race (§7.4.4, ADR-035 D6) — a
  *content* race, not a *pathname* race. rev-0 over-conceded by describing the
  pathname half as unclosable; rev-1 closes it and narrows the disclosure to
  what remains (PIB-103).
- **Not closed: `*os.Root`'s own documented caveats.** `Root.Chmod`,
  `Root.Chown` and `Root.Chtimes` are race-vulnerable on Unix — this design
  calls none of them, and PIB-311 source-scans that. `GOOS=js` and `plan9` do
  not track directories across renames — both are already refused by §7.4.2's
  platform gate.

#### 7.7.3 The single-file writer is not reused for publication

`writeFileAtomicWithRename` (`internal/store/store.go:878-917`) creates its temp
with `os.CreateTemp` and renames with a path-based `os.Rename`, so it is a
path-based writer. It stays exactly as it is and keeps serving every shipped
caller — including `--manual`'s single `status.json` write through
`SaveFeatureStatus`, which is out of the multi-file window by construction
(§6.2). The transactional publication path uses the rooted primitives above
instead. PIB-312 asserts no shipped caller changed; PIB-313 asserts the
publication path never calls it.
...
3. Inspect (accepted inspector, ADR-034 boundaries). Compute and freeze the
   ...
   has been written except the (zero-byte) lock file.
...
12. Release the lock by closing the descriptor. The lock file may remain; it is
    inert (§7.4.2).

Step 11 is the point after which the transaction is invisible. Steps 8→9 are
the armed window; step 9 is the T0 exposure window.
...
| CP1 | lock held, before journal | lock file (inert), maybe a staging tree | acquire; remove staging trees; proceed |
| CP2 | blobs written, before journal | lock file, blobs, no journal | acquire; blobs are additive **orphans**, reported by `doctor` and removable (§9.7.3); proceed |
...
| CP8 | after journal clear, before lock release | inert lock file only | acquire; proceed |
...
| CP10 | `--manual` crash after the lock, before or during the single `status.json` rename | inert lock file; `status.json` either old or new; no journal | acquire; nothing to recover (single-file publication, ADR-035 D3); `FEATURES.md` may be stale and is reconverged by the next transition (§12.3.1) |
...
#### 7.11.1 The T2 boundary: journal loss is unrecoverable, and doctor says so

The journal lives in a gitignored lane, so three ordinary operations destroy
it: `git clean -xfd`, a fresh clone, and any out-of-band `rm -rf .tpatch/local/`.
If that happens **while a transaction was pending**, the preimages are gone with
it and the old bytes are unrecoverable by `tpatch` — except for `--regenerate`,
where the archive blobs are tracked and survive.

This is an explicit boundary of T2 (ADR-035 D21), not an oversight:

- `doctor` can **detect** the symptom (§12.5): a feature whose intent artifacts
  are internally inconsistent, or a lane containing blobs with no index entry.
  It cannot **repair** it, because repair requires evidence that no longer
  exists, and inventing a repair from artifact content would be exactly the
  guess §7.6.3 forbids.
- The disclosure is mandatory in `doctor`'s output and in
  `docs/feature-layout.md`: *"an interrupted `prepare` whose `.tpatch/local/`
  lane was deleted cannot be undone by tpatch; the archived blobs under
  `artifacts/intent-archive/` are the remaining recovery material."*
- Rows: PIB-321 (clean-then-doctor detects and does not repair), PIB-322
  (fresh clone: no journal, no refusal, ordinary preflight), PIB-323 (the
  disclosure string exists in both surfaces).

**Rejected alternative: put the journal under the Git directory.** Storing it
at `.git/tpatch/intent-prepare/<slug>/journal.json` would survive
`git clean -xfd`. It was evaluated and rejected: it does not survive a fresh
clone either (so the boundary moves rather than closes); `.git` is Git's
namespace and writing tpatch control state into it is a compatibility hazard
across worktrees, submodules and the `.git`-as-a-file indirection; a non-Git
workspace needs a second location anyway, so the design would carry two lane
implementations and two sets of crash phases; and `prepare` would gain a hard
Git dependency in exactly the mode (§7.13) this PRD is otherwise making
conditional. Recorded in §21.
...
| `git clean -xfd` during the window | Removes the gitignored lane (journal, staging, lock file). The kernel lock survives on the open descriptor, so this process finishes or fails cleanly; the *next* invocation faces §7.11.1's boundary (PIB-321). |
...
rev-0 claimed "`prepare` performs no Git operation at all". That was false: the
local-lane contract gate shells out to Git and **refuses when Git is
unavailable** (`internal/workflow/session_ignore.go:138-145`), so the claim was
both wrong and, worse, hid a hard Git dependency inside a command that Path B
operators run in workspaces that may not be Git repositories at all.

#### 7.13.1 The exact invocations

| # | Command | Called from | When | Mode(s) |
|---|---|---|---|---|
| G1 | `git rev-parse --is-inside-work-tree` | `gitutil.IsGitAvailable` (`internal/gitutil/ignore.go:25-34`) | before any `.tpatch/local/` write | generate, manual, regenerate, abandon |
| G2 | `git check-ignore -q --no-index -- <lane-leaf>` | `gitutil.IsPathIgnored` (`internal/gitutil/ignore.go:59-78`) via `workflow.EnsureLocalIgnoreContract` | same | same |
| G3 | `git --literal-pathspecs ls-files -- .tpatch/local/` | `rescap.AnythingTrackedUnder` (`internal/rescap/gitgate.go:104-115`) | same | same |
| G4 | `git ls-files -- .tpatch` | `isTpatchUntracked` (`internal/cli/cobra.go:3405-3407`) | only to decide the `bundle-untracked-in-git` advisory | regenerate only |

All four are **read-only**: none stages, commits, checks out, writes the index,
takes `index.lock`, or modifies anything under `.git/`. PIB-107 is a
`git`-exec spy asserting the observed argv set is exactly a subset of
{G1, G2, G3, G4} and that no other Git subcommand is ever invoked; PIB-326 is
its sensitivity fixture.

#### 7.13.2 Non-Git and unusable-Git workspaces

Git presence is decided by a single `Lstat` of `.git` through the held root —
existence only, no content read, no symlink follow, and a `.git` *file* (the
worktree/submodule indirection) counts as present. The behavior table is total:

| `.git` present? | `git` usable (G1 true)? | Behavior |
|---|---|---|
| no | — | The lane gate is **inapplicable**: with no repository, nothing can be tracked and nothing can be ignored. G1–G4 are not run. The command proceeds and emits advisory `workspace-not-git`. `bundle-untracked-in-git` is **not** emitted, because it would be meaningless (PIB-327, PIB-328). |
| yes | yes | The shipped two-target gate runs unchanged (`internal/rescap/scratch.go:47-62`): ignore-covered leaf **and** nothing tracked under `.tpatch/local/`. Failure → exit 3, `local-lane-not-ignored` (PIB-186, PIB-187). |
| yes | no (git missing from `PATH`, or not a work tree) | **Refuse**: exit 3, `local-lane-unverifiable`. A repository exists, so the privacy question is real and unanswerable, and writing into a possibly-tracked lane would violate the boundary the gate exists to enforce (PIB-329). |

**The disclosed regression, and its bound.** In the third row, `tpatch prepare
<slug> --manual` refuses where `tpatch define <slug> --manual` succeeds today.
That is a real behavior difference and it is enumerated as §12.6 D9 rather than
hidden. It is bounded three ways: it requires a Git repository whose `git` is
unusable (an unusual combination); the per-phase `--manual` commands remain
available and unchanged, so Path B still has a complete route (§12.4); and the
refusal's remediation names both alternatives explicitly. Making the gate
*optional* in that row was rejected — it would let a mutating command write
into a lane that may be tracked, which is exactly what ADR-027's boundary
forbids.

#### 7.13.3 What Git is never used for

`prepare` never uses Git as a preservation mechanism, a recovery mechanism, or
an authority on artifact content. G4's advisory tells the operator that
committing `.tpatch/` would add a second recovery route; the archive is the
first one and does not depend on Git (§8.2 H2, ADR-035 D8).
...
as **Proposed rev-1**, keeps it in the ADR index, and requires it to be
...
| `regenerate` with ≥1 present artifact | yes | one per distinct replaced content | exactly one generation, idempotently (§9.3) | on demand, lazily |
  ...
  "generations": [
    {
      ...
      "mode": "regenerate",
      "replaced": [
        { "artifact_id": "analysis",         "path": "analysis.md",             "blob": "a91e…", "size_bytes": 4211, "purged": false },
        { "artifact_id": "analysis_sidecar", "path": "artifacts/analysis.json", "blob": "c40a…", "size_bytes": 733,  "purged": false },
        { "artifact_id": "spec",             "path": "spec.md",                 "blob": "",      "size_bytes": 1902, "purged": true  }
      ]
    }
  ]
}
...
- `schema_version: 1` (ADR-035 D10). Fixed key order by Go struct declaration order — the
...
- `generation_id` = lowercase-hex SHA-256 over the **canonical** encoding of
  `{feature, mode, replaced[]}` with `replaced` sorted by `artifact_id` and
  computed over the **pre-purge** shape (so purging never changes an existing
  id). It is content-addressed and deterministic — the shipped `ComputeBatchID`
  shape (`internal/store/resource_publish.go:155-167`), whose derivation lives
  at exactly one point so every consumer agrees by construction
  (`internal/store/resource_publish.go:131-143`).
- `replaced` is sorted by `artifact_id`; `generations` is in publication order.
- `purged` is the tombstone flag (§9.7.2): `false` for every entry a
  publication writes, and only ever flipped to `true` — with `blob` cleared —
...
- A `generation_id` collision between two different canonical bodies refuses
  ...
  `batch-id-collision` disposition (`internal/store/resource_publish.go:198-202`).

#### 9.3.1 Strict decoding — the index is never guessed at

Every read of `index.json` — by a publication, by a purge, by `list`, by
`doctor` — uses one strict decoder, and every failure is a named refusal that
writes nothing. The shipped precedent is
`internal/store/resource_publish.go:305-328` (unknown fields, trailing content
and null arrays all refused).
...
| X3 | `schema_version` equals this build's constant; a **higher** version refuses with a distinct message telling the operator to upgrade `tpatch` rather than suggesting the file is broken | `archive-index-version-unsupported` | 3 |
| X4 | `feature` equals the slug whose directory it was read from | `archive-index-foreign` | 3 |
| X5 | `generations` and `replaced` are arrays, never `null` | `archive-index-corrupt` | 3 |
| X6 | every `generation_id` matches `^[0-9a-f]{64}$` and is unique in the file | `archive-index-corrupt` | 3 |
| X7 | every `blob` is `""` or matches `^[0-9a-f]{64}$`; `blob == ""` **iff** `purged == true` | `archive-index-corrupt` | 3 |
| X8 | every `path` is a relative, non-escaping path inside the feature directory | `archive-index-path-escape` | 3 |
| X9 | every `artifact_id` is in the closed four-value set (`analysis`, `spec`, `exploration`, `analysis_sidecar`) and unique within its generation | `archive-index-corrupt` | 3 |
| X10 | recomputing `generation_id` over the pre-purge canonical body reproduces the recorded value | `archive-index-generation-mismatch` | 3 |

A corrupt index never causes a rewrite, never causes a blob removal, and never
causes a canonical write: the invocation refuses and names the file. Rows:
PIB-331 … PIB-340, plus PIB-341's sensitivity fixture (a valid index with one
injected unknown key must fail).
...
`home-absolute-path` in particular can match a legitimate example path inside a
hand-authored spec. The chosen trade is: a false positive costs the operator an
...
1. CAS the blob file: it must be present and its bytes must hash to its name
   (§7.6.3). A corrupt blob refuses the whole purge (`archive-blob-corrupt`);
   an already-absent blob is a no-op for that selector, so purging twice is
   idempotent (PIB-351).
2. Rewrite `index.json` **first**, through one single-file rename — atomic in
   the ordinary POSIX sense, the one case ADR-035 D1 and D3 allow the word —
   flipping every `replaced` entry that references the blob to
   `{"blob": "", "purged": true}` and leaving `artifact_id`, `path`,
   `size_bytes` and the enclosing `generation_id` untouched.
3. Then remove the blob file and fsync `blobs/`.

The order is load-bearing: an index that still references a removed blob would
be a lie, whereas a tombstoned index plus a still-present blob is merely a
harmless orphan that the next `purge --orphans` collects. A crash between (2)
and (3) therefore lands in the recoverable direction (PIB-352, PIB-353).

**Tombstones, not deletions of history.** The generation record survives with
its `generation_id` unchanged, so the archive never forgets *that* a
replacement happened — it only forgets the bytes, which is what the operator
asked for. `--generation <id>` purges every blob that generation references;
`--all` purges every blob in the archive and tombstones every entry;
`index.json` itself is never removed, so a purged archive is an honest,
readable record of what it no longer holds (PIB-354, PIB-355).

**Recovery constraint, stated plainly.** A purged blob is gone. `list`,
`doctor` and the purge report all say so in the same words: *"purged entries
record that a replacement happened; their bytes are not recoverable by
tpatch."* No route in this design resurrects them, and none pretends to
(PIB-356).

**Shared blobs are reference-counted before removal.** Content addressing means
two generations can name the same blob. A purge that would remove a blob still
referenced by a **non-selected** entry refuses that blob with
`archive-blob-referenced`, names the referencing generations, and continues
with the rest; `--all` has no such case by construction (PIB-357, PIB-358).

#### 9.7.3 Orphans

An orphan is a `blobs/*.blob` file that no index entry references. Orphans come
from crashes between §7.8 step 7 and step 9's index rename, and from refusals
after a blob write.
...
### 12.5 `doctor` gains one check, and it covers every residue

A new diagnostic check reports — and never acts on — the transaction and
archive residues this design can leave. It is the ninth check, following the
shipped D1…D8 registry (`internal/workflow/doctor.go:226-237`).

- **ID**: `D9`. **Severity**: warning, never blocking (Q8).
- **Never**: takes the transaction lock in a way that could block another
  process (it probes without creating; see below), reads artifact content,
  mutates anything, or changes `doctor`'s exit code from what the other checks
  decide.

**What it reports, per slug — the total residue set:**

| Residue | Detected by | Reported as |
|---|---|---|
| Pending transaction | `journal.json` exists in the slug's lane | "a mutating `prepare` was interrupted; running `tpatch prepare <slug>` (any mutating mode) recovers it first" |
| **Live** transaction | a **non-creating** lock probe contends (§12.5.1) | "a `tpatch prepare` is running for this feature right now" — and D9 reports nothing else for that slug, because every other observation would be a race |
| Orphan lock file | `prepare.lock` exists but the probe does **not** contend | reported as **informational and harmless**: "the lock file is inert; no process holds it" — never as a defect, never with a removal instruction, because rev-1's lock file carries no authority (§7.4.2) |
| Retained staging trees | `stage-*` directories exist | how many, their repo-relative paths, and that the next successful mutating `prepare` removes them |
| Abandoned evidence | `abandoned-*` directories exist (§6.6) | how many, their paths, and that removing them is `rm -rf <dir>` |
| Orphan archive blobs | blob files no `index.json` entry references (§9.7.3) | how many, and the exact `tpatch feature intent-archive purge <slug> --orphans --yes` command |
| Corrupt archive index | any X1–X10 bind fails (§9.3.1) | the failing bind's code and the file path; **no** repair suggestion beyond `list`, because repairing a corrupt tracked JSON is a human decision |
| Journal-loss symptom | blobs and/or a mixed artifact set with **no** journal (§7.11.1) | that an interrupted `prepare` may have lost its undo record, that tpatch cannot recover it, and that `artifacts/intent-archive/` holds whatever prior bytes were archived |

#### 12.5.1 The lock probe is read-only

D9 must be able to say "a prepare is running" without becoming a writer. The
probe therefore opens the lock file **read-only and without `O_CREATE`**
(`OPEN_EXISTING` with deny-share on Windows), attempts the non-blocking
exclusive acquisition, and releases it immediately on success. `ENOENT` means
no lock file and therefore no live holder.

- It never creates the lock file, never writes a byte, and never leaves the
  lock held (PIB-380).
- `doctor`'s zero-mutation property is asserted with a whole-tree byte snapshot
  plus a filesystem spy proving no `O_CREATE` on the lock path (PIB-135,
  PIB-381).
- The probe's momentary acquisition can make a *concurrent* `prepare` see
  contention for the duration of a syscall. That is disclosed in §16 R11 and
  is the reason `prepare`'s contention refusal is a transient exit 3 with a
  "wait and retry" remediation rather than a terminal state.

Acceptance: PIB-133 (fires on a pending journal), PIB-134 (silent when the lane
is clean), PIB-135 (zero mutation), PIB-136 (D1…D8 outputs byte-identical
against pre-change goldens), PIB-382 … PIB-386 (one row per residue class),
PIB-387 (totality: the residue table is a total function over the observable
lane and archive state).
...
| D8 | **Mutating** `prepare` refuses on platforms with no process-lifetime lock primitive (`solaris`, `aix`, `js`, `plan9`, `wasip1`), which is narrower than `--check`'s `unix \|\| windows` read allowlist. `--check` is unaffected on those targets (§7.4.2) | `prepare` (mutating modes only) |
| D9 | **Mutating** `prepare` refuses with `local-lane-unverifiable` when `.git` exists but `git` is unusable, where `define --manual` succeeds today. Non-Git workspaces are unaffected and proceed with an advisory (§7.13.2) | `prepare` (mutating modes only) |
...
| D12 | The shipped `rescap` kernel lock is extracted into a shared package and gains a Windows sibling; `rescap`'s own behavior, refusal codes and platform envelope are unchanged and golden-pinned (§7.4.2) | `feature resource *` (internal refactor only) |
...
### 13.2 Writes: rooted primitives, no symlink following, non-regular refusal
...
2. **Every write is handle-relative** — `(*os.Root).OpenFile`,
   ...
   ancestor rename can redirect a write outside the tree, and no path-based
   `os.Rename`/`os.CreateTemp`/`os.WriteFile` follows a rooted check (§7.7,
   PIB-308, PIB-310).
3. **Publication targets are never followed.** Immediately before each rename,
...
| `.tpatch/local/intent-prepare/<slug>/prepare.lock` | **gitignored** | inert file; the kernel lock lasts one invocation (§7.4.2) |
...
| Feature whose `.tpatch/` is untracked in Git | works; advisory `bundle-untracked-in-git`, and no clone-durability is claimed for the archive (PIB-139, PIB-342) |
| Workspace that is not a Git repository at all | works in every mutating mode; the local-lane gate is inapplicable; advisory `workspace-not-git` (PIB-327) |
| Git repository whose `git` binary is unusable | mutating modes refuse with `local-lane-unverifiable`; the per-phase `--manual` commands still work (PIB-329) |
| Workspace on a platform with no process-lifetime lock primitive | mutating modes refuse with `prepare-unsupported-platform`; `--check` is unaffected (PIB-288, PIB-289) |
...
| R9 | A future reviewer assumes a test proves semantics because it exists. | §18.1's disqualifying assertion shapes; §18.41's sensitivity and semantic-fixture requirements over every guard row. |
...
| R12 | A false-positive redaction match blocks a legitimate regeneration. | The trade is stated in §9.6.1: an edit and a re-run versus a credential in every future clone. The refusal names the artifact and class codes so the operator can see what matched; Q9 records a scoped-override design, deliberately not in v1. |
| R13 | The lock extraction regresses `feature resource *`. | S0b lands pre-change goldens for every `feature resource` subcommand before the extraction; PIB-286 and PIB-287 compare against them, including refusal text and the unsupported-platform envelope. |
...
| R15 | A journal lost to `git clean`/fresh clone is read as a tpatch bug. | §7.11.1 states the boundary, `doctor` reports the symptom with the same wording, and `docs/feature-layout.md` carries the disclosure (PIB-321, PIB-323). |
...
3. **S0b — the lock extraction goldens.** Before `internal/rescap`'s lock is
...
| **S1b** | Lock extraction: `internal/proclock` (unix `flock` moved from `internal/rescap/lock_unix.go`, new Windows deny-share sibling, fail-closed stub), plus the `rescap` wrapper that keeps its behavior byte-identical. | new `internal/proclock/**`; modified `internal/rescap/lock_unix.go`, `internal/rescap/lock_unsupported.go` |
...
| **S3** | The archive: blobs, index, canonical encoding, `generation_id` derivation, idempotent append, X1–X10 strict decode, corruption/collision refusals, the redaction gate, tombstones, orphan detection. | new `internal/store/intent_archive.go` |
| **S4** | CLI wiring: modes, flag mutexes, precedence, report model, renderers, exit codes, advisories, `--dry-run`, `--abandon-transaction`. | modified `internal/cli/prepare.go` (the file the accepted S3 creates), new `internal/cli/prepare_publish.go` |
...
`failFsync(path)`, `failRename(path)`. Each is a function-valued package
...
| PIB-053 | I | `--manual` while a live process holds the lock | exit 3, `transaction-in-progress`; zero mutation; the remediation says to wait and retry |
...
| PIB-079 | C | `--dry-run` with a pending journal | exit 3, `recovery-pending`; recovery does **not** run; the journal is byte-identical afterwards; no plan is printed |
...
| PIB-117 | C | crash phase CP1 (lock file present, no journal) | the next run acquires the kernel lock without inspecting the file; staging removed; run proceeds to exit 0 |
| PIB-118 | C | crash phase CP2 (blobs, no journal) | blobs remain as reported orphans; the run proceeds to exit 0; no index entry was added; advisory `archive-orphan-blobs` names the purge command |
...
| PIB-124 | I | a second mutating `prepare` while a live process holds the lock | exit 3, `transaction-in-progress`; the first run's outcome unaffected |
...
| ID | Kind | Case | Asserted observable |
|---|---|---|---|
| PIB-268 | I | `--dry-run` with a pending journal, all three modes | exit 3, `recovery-pending`; no plan printed; the journal, staging tree and lock file byte-identical |
...
| ID | Kind | Case | Asserted observable |
|---|---|---|---|
| PIB-279 | I | Git-backed workspace, `.tpatch/local/` not covered by an ignore rule | exit 3, `local-lane-not-ignored`, before any staging byte and before the lock file is created |
...
| PIB-282 | I | lane gate passes | the lock file is created only after both halves pass |
...
| PIB-284 | I | a leftover `prepare.lock` from a killed process | the next mutating run succeeds; the file's presence causes no refusal and no message |
| PIB-285 | S | the lock implementation | the lock file is opened but never written; no nonce, PID or hostname is ever put in it |
...
| PIB-288 | U | mutating `prepare` on a build target with no lock primitive | exit 3, `prepare-unsupported-platform`, before any root is opened and before any write |
| PIB-289 | U | `prepare --check` on the same target | unchanged accepted behavior; ADR-034's `unix \|\| windows` allowlist still governs the read half |
| PIB-290 | I | two live mutating `prepare` processes, same slug (cross-process, real binaries) | the second exits 3 with `transaction-in-progress`; the first completes normally |
| PIB-392 | U | a workspace whose lock-lane filesystem is outside the shipped allowlist | exit 3, `lock-filesystem-unsupported`, before the lock file is created and before any other write; `--check` on the same tree is unaffected |
| PIB-393 | G | the shipped docs and `prepare --help` | both carry the Windows network-share share-mode sentence of §7.4.2; a sensitivity fixture removing it fails the guard |
| PIB-394 | G | the refusal catalog of §10.4 | `prepare-unsupported-platform` and `lock-filesystem-unsupported` are distinct codes with distinct messages, and neither is emitted by `--check` |
...
| PIB-310 | C | an ancestor directory (`features/` or `<slug>/`) is renamed between preflight and publication | the rooted write fails closed; nothing is written outside the original tree; exit 5 |
| PIB-311 | S | the publication path | `Root.Chmod`, `Root.Chown` and `Root.Chtimes` are never called |
| PIB-312 | G | `internal/store/store.go` | `writeFileAtomicWithRename` is unchanged byte-for-byte and every shipped caller still reaches it |
| PIB-313 | S | the transactional publication path | it never calls `writeFileAtomic`/`writeFileAtomicWithRename` |
...
| PIB-321 | I | `git clean -xfd` between a crash and the next run | the next run proceeds without a journal; `doctor` D9 reports the journal-loss symptom and repairs nothing |
| PIB-322 | I | fresh clone of a repo whose `.tpatch/` is committed mid-transaction | no journal, no refusal; ordinary preflight; the archive blobs are present |
| PIB-323 | G | `doctor` output and `docs/feature-layout.md` | both carry the journal-loss disclosure sentence; a sensitivity fixture removing it fails the guard |
...
| PIB-327 | I | mutating `prepare` in a directory tree with `.tpatch/` but no `.git` | exit 0; advisory `workspace-not-git`; the git-exec spy records **zero** invocations |
| PIB-328 | I | the same workspace under `--regenerate` | `bundle-untracked-in-git` is **not** emitted; `workspace-not-git` is |
| PIB-329 | I | `.git` present, `git` absent from `PATH`, every mutating mode | exit 3, `local-lane-unverifiable`; the remediation names the per-phase `--manual` commands; zero mutation |
...
| PIB-340 | U | a `generation_id` that does not reproduce from its body (X10) | exit 3, `archive-index-generation-mismatch` |
...
| PIB-349 | I | `purge` while a live `prepare` holds the lock | exit 3, `transaction-in-progress`; zero writes |
...
| PIB-352 | C | `purge --blob <h> --yes` with a crash injected between the index rewrite and the blob removal | the index is tombstoned and the blob remains as an orphan; a later `purge --orphans` collects it; no state claims bytes it does not have |
...
| PIB-354 | I | `purge --generation <id> --yes` | every entry of that generation is `{"blob": "", "purged": true}`; `generation_id` unchanged; `index.json` still decodes strictly |
...
| PIB-380 | C | `doctor` with a live `prepare` running, filesystem spy | the lock file is never created by `doctor`; the probe holds nothing after it returns; D9 reports "running now" |
| PIB-381 | C | `doctor` with no lock file present | no lock file is created; D9 reports no live transaction |
...
| PIB-386 | I | `doctor` with an inert lock file and nothing else | D9 reports it as informational; it is not a warning and does not suggest removal |
...
### 18.40 Counts, kinds and slice partition

- **394 rows**, `PIB-001`…`PIB-394`, contiguous, zero duplicates, zero retired.
  160 rows are new in rev-1 (`PIB-235`…`PIB-394`); 15 rev-0 rows were amended
  in place and kept their IDs (§18.1).
- **38 categories**: A 20, B 24, C 15, D 12, E 9, F 19, G 13, H 14, I 13, J 8,
  ...
  AK 10, AL 4. Sum = 394.
- **By kind**: `I` 179, `C` 79, `G` 72, `U` 43, `S` 21. Sum = 394. Every row
  whose kind is `G` is covered by §18.41's sensitivity requirement.
...
Sum = 394; zero unassigned, zero double-assigned (PIB-230's ledger check
derives this partition mechanically rather than trusting the table).

### 18.41 Sensitivity requirement
...
| Q12 | Should mutating `prepare` support `solaris`/`aix` through a different lock primitive? | Those targets have no `syscall.Flock` in the pinned toolchain, so supporting them means POSIX record locks with their own well-known ownership pitfalls. | Refuse (§7.4.2, §12.6 D8). Lifting it requires CI coverage for the target, as `rescap` already requires. |
| Q13 | Should a repository on a non-allowlisted filesystem be able to opt in to mutating `prepare`? | Fail-closed is right for a lock the recovery model depends on, but it blocks network-mounted workspaces entirely (§7.4.2). | Refuse. A relaxation needs its own evidence that the filesystem honours `flock`, not a flag. |
...
| Path-based `os.Rename` after rooted checks (rev-0's publication) | **Rejected.** `(*os.Root).Rename` exists in the pinned toolchain (`go.mod:3`), so the ancestor-swap race had no reason to exist (§7.7). |
