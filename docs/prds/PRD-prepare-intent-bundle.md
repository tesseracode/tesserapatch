...
no-decision erratum (2026-08-20); rev-17 observed-product companion-row
no-decision erratum (2026-08-27), rev-18 removal-identity-re-probe
no-decision erratum (2026-08-28) and rev-19 observed-product surface-split
no-decision erratum (2026-08-28) — rev-17, rev-18 and rev-19 accepted
jointly on 2026-08-29; rev-20 selector-classification bounded public
...
**Rev-17 acceptance**: 2026-08-29 — **Accepted no-decision erratum**, jointly
with rev-18 and rev-19 (raised 2026-08-27); AU implementation discovery
corrected **three** acceptance-matrix rows whose stated fixture expectations the
already-accepted rules and the shipped product do not produce: `PIB-542`'s
first fixture contradicted the rev-12/ADR-035 D16 sole-class admission rule,
`PIB-543` stated one exit and one procedure presentation for five observers
whose product truth differs by surface, and `PIB-544` named a driver seam set
that is not the one the four §9.7.2 windows are actually reached through. It
changes no decision, no normative rule, no row ID, kind, category, count,
command, exit code or state that the product emits; the matrix remains **567**
rows with **thirty-six** semantic guards.
**Rev-18 acceptance**: 2026-08-29 — **Accepted no-decision erratum**, jointly
with rev-17 and rev-19 (raised 2026-08-28); S7 AV implementation discovery
corrected **one** acceptance-matrix row, `PIB-550`, and the §9.7.2 window row
it tests.
Both said an external replacement landing between the pre-removal revalidation
and the unlink is removed undetected. The shipped removal is not a bare unlink
of a revalidated path: `prepareArchiveStorage.RemoveBlob` re-probes the managed
object's identity and refuses the removal when it no longer matches the identity
step 2 recorded, so the replacement is preserved and the invocation ends at
exit **5** `archive-purge-partial`, with the sanitized rerun at exit **6**
`archive-purge-evidence-divergent`. The disclosed residual narrows to the
unseamed gap between that re-probe and the `root.Remove` syscall, which is still
neither detected nor claimed closed. It changes no decision, no normative rule,
no row ID, kind, category, count, command, exit code or state that the product
emits; the matrix remains **567** rows with **thirty-six** semantic guards.
**Rev-19 acceptance**: 2026-08-29 — **Accepted no-decision erratum**, jointly
with rev-17 and rev-18 (raised 2026-08-28); the independent S7 AX review found
**three** record inconsistencies between this document, the already-accepted
rules and the shipped product. **Exactly two acceptance-matrix rows are amended**:
`PIB-562`, whose closing sentence stated one exit for `list` and `doctor`
jointly, and `PIB-566`, whose derived emitter population omitted `doctor` D9's
pending-purge remediation and asserted a `retry_cwd` field on a carrier that
has none. The third correction touches **no** matrix row: §10.2's worked
sequential-repair example showed an exit-0 `purged` report whose plan still
carried the rank-1 `corrupt-object` manual prerequisite, which §9.3.1's own
precondition makes unreachable, so it becomes the coherent exit-3
archive-integrity refusal form. §16's `R25`, `PIB-566`'s §18.53
semantic-fixture wording and ADR-035's paired D10 paragraph are corrected to
the same truth as non-matrix companions. It changes no decision, no normative
rule, no row ID, kind, category, count, command, exit code or state that the
product emits; the matrix remains **567** rows with **thirty-six** semantic
guards.
...
outside D16's diagnostic vocabulary. It is recorded as
an amendment rather than as a no-decision erratum because the closed public
code a purge selector that names nothing emits **changes**: the store already
classifies a `--blob`/`--generation` value the index does not carry as
`archive-selector-invalid`, while the shipped CLI rewrote that classification
to `archive-index-corrupt` and the store's own unknown-generation branch used
the same archive-integrity code, so a feature with **no** archive and an
unknown but well-formed hash was reported as a corrupt archive and told to
preserve bytes that do not exist. rev-20 surfaces the existing code for exactly
that population — a well-formed lowercase 64-hex `--blob` hash the index
carries no reference to, and a well-formed `--generation` id it records no
generation for — and gives it a selector-specific message and remediation.
Selector **grammar** is untouched and stays outside the catalog: a missing or
repeated scope family, and a value that is not a full lowercase SHA-256, remain
exit-1 usage errors the shipped command rejects before any archive read, with
no report and no refusal code. The D1–D21 architecture is otherwise unchanged:
no mode, transaction, admission condition, class rank, repair route, lock, Git
or report shape moves, the matrix stays at **567** rows, no row is added,
retired, renumbered, re-categorised or re-kinded, §18.52's category/kind/slice
arithmetic is unchanged, and §18.53 stays at **thirty-six** semantic guards.
...
pending-owner wording correction discovered during S7 implementation; rev-17
`PIB-542`/`PIB-543`/`PIB-544` observed-product corrections discovered during
S7 AU implementation (`PIB-537`…`PIB-545`); rev-18 `PIB-550` removal-identity
re-probe correction discovered during S7 AV implementation
(`PIB-546`…`PIB-551`); rev-19 `PIB-562`/`PIB-566` observed-product corrections
and the §10.2 worked-example coherence correction raised by the independent S7
AX review (`PIB-561`…`PIB-567`); rev-20 selector-classification amendment
...
(**Accepted 2026-08-14**, rev-20; its companion rev-17, rev-18 and rev-19
errata were accepted in the same joint review as this document's, on
2026-08-29, and its paired rev-20 D16 diagnostic-vocabulary amendment was
accepted with this document's rev-20 on 2026-08-30). **This PRD and
ADR-035 were reviewed and accepted together.** ADR-035 locks the
publication/history decisions; this PRD states the product contract that
depends on them. Where they overlap, **ADR-035 is normative**.
...
> `origin/main` before GH #23 recorded WAVE_BASE `3b579fc`. rev-15's evidence
> correction was jointly approved on 2026-08-18, which authorized the mutating
> production slices; GH #23 implemented §17.2 S1–S7 and that implementation
> has been accepted through aggregate review. The correction changed only how
> PIB-391 proves which binary produced the frozen `--check` bytes; it changed
> no command behavior, decision, matrix row, kind or count.
...
- [ADR-035 intent bundle publication and history](../adrs/ADR-035-intent-bundle-publication-and-history.md) — **companion, Accepted rev-14 (2026-08-14)**; the transaction, the archive and the honesty limits
...
| rev-17 | **Accepted no-decision erratum — raised 2026-08-27, accepted 2026-08-29** | S7 AU implementation (`PIB-537`…`PIB-545`) proved that **PIB-542's first fixture stated an expectation the accepted rules do not produce**. That fixture's archive holds exactly one repair class — the mixed tombstone/live-reference hash `h₂` — beside a healthy `h₃`, yet the row said *no* selector is any class's admitted repair and expected **all four** to exit 3. Under the frozen rev-12 admission rule, restated in ADR-035 D16, `--all --yes` is admitted precisely when the chosen class is the archive's **only** class, and a healthy hash belongs to **no** class and therefore cannot supply a second one. The corrected row keeps `--orphans`, `--blob <h₃>` and `--generation <id₃>` refusing exit 3 zero-write — none of them is any class's repair — and states that `--all --yes` **is** admitted, carries its mandatory whole-archive blast-radius disclosure (PIB-557) and purges the whole archive at exit 0. The clean-control fixture and the two-mixed-hash fixture are unchanged, and the fourth fixture is unchanged because a removable unreferenced residue beside `h₂` really is a **second** class, so `--all` still refuses there while `--orphans --yes` is admitted. **`PIB-543` stated one exit and one procedure presentation for five observers whose product truth differs by surface.** The row said “every observer exits **3**”, but D16 already fixes `doctor`'s D9 as **warning-only**, and the shipped `DoctorExitCode` returns **0** when a run raises only warnings: the corrected row keeps the four mutating/inventory observers — `prepare`, `--regenerate`, `--manual` and `list` — at exit **3** `archive-blob-corrupt` with the whole-tree byte snapshot, and states that `doctor` renders the identical corrupt-object observation as a **warning** finding and exits **0**. The row also required the same rendered procedure from all five; the product prints the same route in two presentations. `list` renders the archive's own procedure per entry — the destructive warning, the single POSIX-quoted `rm -rf -- '<managed blob path>'`, and the structured `retry` with `retry_cwd: "workspace-root"`; `doctor`'s D9 remediation renders the same quoted warning/removal lines plus the `After every removal above, run <the literal purge retry>` line, the restore alternative and the Git-history caveat, without a separate `retry_cwd` field; and `prepare`, `--regenerate` and `--manual` render the same route as the refusal's `remediation` prose — the destructive warning, `rm -rf -- <repo-relative managed blob path>` (unquoted; the path is hash-addressed and holds no shell metacharacter), the literal `tpatch feature intent-archive purge <slug> --blob <h> --yes`, the restore alternative and the Git-history caveat — and carry no `retry`/`retry_cwd` pair. Every D16 obligation on those procedures is unchanged and still asserted on **all five**: repo-relative paths only, the stated destructive cost, the Git-history caveat, the single exact-path `rm -rf --` with its `--` terminator and no wildcard, no preservation command, no `--orphans`, and none of the ten forbidden command words. The sixth owned fixture (exit-6 `archive-purge-evidence-divergent`, PIB-558), the five object kinds and both executed repair routes are unchanged. **`PIB-544` named a driver seam set that is not the one the four §9.7.2 windows are reached through.** The row named `beforePurgeIndexCAS`, `afterPurgeBlobRevalidate`, `beforePurgeBlobRemove` and `beforePendingTombstoneCAS`; the authoritative seams are the ones the lower store fixture `TestRecoverPendingPurgeInsertionWindowsPIB544` already drives, and the corrected row states that mapping: window 1 (before the claim's re-read) is the index **capture** taken after the recovery preflight — store `before:capture-index` on its second capture, at CLI level a `CaptureIndex` storage spy armed once `PreflightIndexCAS` has run; window 2 (between the re-read and its CAS) is `beforePurgeIndexCAS` (store `before-index-cas`); window 3 (between the CAS and the pre-removal revalidation) is `afterPurgeIndexRename` (store `after-index-cas`), immediately after the claim's CAS publication; and window 4 (after the removal, before the tombstone CAS) is the post-unlink seam `beforePendingTombstoneCAS` (store `after-remove`). `afterPurgeBlobRevalidate` is **not** a driver for windows 1–4: it fires after the pre-removal revalidation, which is the **fifth** window PIB-550 owns and this row explicitly excludes; `beforePurgeBlobRemove` observes the pre-removal syscall rather than opening an insertion window of its own. **No window's behaviour changes**: window 1 still completes with the inserted reference claimed and tombstoned, windows 2 and 3 still perform no removal and end `archive-purge-partial` at exit 5 with `resume: "pending-recovery-then-completion"` and `pending_hash: h`, window 4 is still detected by the tombstone CAS with its retry taking the absent-blob path, the sanitized retries still complete at exit 0, and the fifth fixture still refuses exit 3 `archive-purge-index-changed` with a whole-tree byte snapshot. **Scope of this erratum**: no decision, normative rule, admission condition, precedence rank, exit code the product emits, state, vocabulary token, class ranking or stage arithmetic is added, withdrawn or reopened; ADR-035's D16 normative text is untouched; no row is added, retired, renumbered or re-kinded and no row changes category; the matrix stays at **567** rows, §18.52's arithmetic is untouched and §18.53 stays at **thirty-six** semantic guards. **Amended rows, exactly**: `PIB-542`, `PIB-543`, `PIB-544` — **three**. |
| rev-18 | **Accepted no-decision erratum — raised 2026-08-28, accepted 2026-08-29** | S7 AV implementation (`PIB-546`…`PIB-551`) proved that **`PIB-550` and the fourth row of §9.7.2's window table stated an observable the shipped storage does not produce**. Both said that a replacement of the managed object landing between step 2's revalidation and step 3's unlink is removed and that the byte-level loss is undetected. The shipped removal is not a bare unlink of a revalidated path: `removeIntentArchiveBlob` hands the revalidated identity to `prepareArchiveStorage.RemoveBlob`, whose `PreflightBlobRemove` re-probes the managed object and refuses the removal when the identity no longer matches — for a regular blob that identity is the recorded mode, size and content hash, so only a byte-identical object passes it. A replacement injected at `beforePurgeBlobRemove` is therefore **preserved**: the invocation ends at exit **5** `archive-purge-partial` with `resume: pending-recovery-then-completion`, `pending_hash: h`, zero completed hashes and `h` still removal-pending, and the sanitized rerun refuses exit **6** `archive-purge-evidence-divergent` naming the managed blob path. The genuinely open residual narrows to the **unseamed gap between that re-probe and the `root.Remove` syscall**, which has no injection point, is the same disclosed final-syscall residual canonical publication carries, and is still neither detected nor claimed closed. The erratum amends `PIB-550`, §9.7.2's fourth window row and its closing residual paragraph, §9.7.2's exit-6 atomicity note, §21's corresponding alternatives row, and the §18.53 sensitivity entries of `PIB-546` and `PIB-551`, which gain the fixtures the same review required; ADR-035 rev-18 carries the paired D16 correction. No decision, normative rule, row ID, kind, category, count, command, exit code or emitted state changes; the matrix remains **567** rows, §18.52's arithmetic is untouched, and §18.53 remains at **thirty-six** semantic guards. |
| rev-19 | **Accepted no-decision erratum — raised 2026-08-28, accepted 2026-08-29** | The independent S7 AX review proved **three** record inconsistencies between this document, the already-accepted rules and the shipped product; the erratum corrects exactly those and nothing else. **`PIB-562` stated one exit for two surfaces jointly.** Its closing sentence said `list` and `doctor` render the corrupt object and both residues in one pass at exit **3**. `doctor`'s D9 is warning-only — ADR-035 D16 already says so, the accepted rev-17 `PIB-543` correction restates it, and the shipped `DoctorExitCode` returns **0** when a run raises only warnings. The corrected row keeps `list` rendering every observation in one pass at exit **3** and states separately that `doctor` renders that identical observation set as **warning** findings at exit **0**; the rest of the row — the two-stage/one-invocation totals, the `stages[0].resulting_classes` of `[]`, the byte-identical index and the prohibition on naming `--orphans --yes` as the corrupt object's repair — is unchanged. **`PIB-566` omitted an emitter and over-scoped a field.** Its derived population left out §12.5's `doctor` D9 pending-purge remediation, which observes the same removal-pending reference set and prints the same narrow sorted command, and it required `retry_cwd: "workspace-root"` from **every** emitter although `doctor` carries the command as remediation prose with no `retry`/`retry_cwd` pair — the carrier truth the accepted rev-17 `PIB-543` correction already records. The corrected row adds the `doctor` emitter and scopes `retry_cwd` to the structured retry carriers, keeping the no-`--all`, exact-pending-set, no-inherited-`--path` and narrow-versus-`--all` recovery-equivalence obligations on every relevant carrier, and keeps **three** semantic sensitivity fixtures. **§10.2's worked sequential-repair example was internally impossible.** It showed `outcome: "purged"` with `repaired_class: "unreferenced-residue"` beside a plan whose `stages[0]` is still the rank-1 `corrupt-object` `manual-prerequisite`; §9.3.1's precondition refuses **every** confirmed selector zero-write while any `corrupt-object` instance is present, so no invocation over that archive can have repaired a class. The example becomes the exit-3 archive-integrity refusal form the same paragraph already describes — `outcome: "refused"`, `action: "none"`, no `repaired_class` — preserving the corrupt-object manual stage that makes it worth printing, and agreeing with the standing rule that `repaired_class` is present if and only if the invocation repaired a class. §16's `R25`, `PIB-566`'s §18.53 semantic-fixture wording and ADR-035's paired D10 paragraph are corrected to the same surface split; ADR-035 rev-19 carries the D10 half and changes no D10 or D16 decision. **No decision**, normative rule, admission condition, precedence rank, row ID, kind, category, count, command, exit code, emitted state or vocabulary token is added, withdrawn or reopened; the matrix remains **567** rows, §18.52's arithmetic is untouched, and §18.53 remains at **thirty-six** semantic guards. **Amended rows, exactly**: `PIB-562` and `PIB-566` — **two**. |
| rev-20 | **Accepted bounded public diagnostic-vocabulary amendment — 2026-08-30** | The post-close external review of the GH #23 cluster proved that the **public code a purge selector that names nothing emits is wrong**, in two places that pointed at each other. `internal/store` already classifies a `--blob`/`--generation` value the index does not carry as `archive-selector-invalid` and returns it with the selector's own typed value, but the shipped CLI refusal builder **rewrote** that code to `archive-index-corrupt` because the closed §10.4.1 catalog did not contain it, and the store's adjacent unknown-generation branch emitted `archive-index-corrupt` directly. The observable defect: a feature with **no archive at all** and a well-formed but unindexed hash was told its archive index was corrupt and to *preserve the archive bytes* — bytes that do not exist — with no route to the listing that would have shown the selector named nothing. This revision is recorded as a **bounded public diagnostic-vocabulary amendment, not a no-decision erratum**, because the emitted closed code changes and §10.4.1's catalog grows from **53** to **54** codes. **What changes**: the existing exact code `archive-selector-invalid` is surfaced rather than rewritten — no second name is invented; it is the emitted code for exactly two public populations, a well-formed lowercase 64-hex `--blob` hash the index carries no reference to and a well-formed `--generation` id it records no archive generation for, both at **exit 3**, zero-write, in the preview and the confirmed form alike; and the refusal gains a selector-specific message and remediation that names `tpatch feature intent-archive list <slug>` from the workspace root and says to choose a listed value or stop when the listing names none. **What does not change**: selector **grammar** — a missing or repeated scope family, and a `--blob`/`--generation` value that is not a full lowercase SHA-256 — stays an **exit 1** usage error that the command's own scope check and selector normalization reject before any archive is read, with an empty report, no refusal code and no echo of the rejected value, so it is not a catalog population and `PIB-347` is not amended; the store keeps its own defensive classification of the arity, malformed-value, unusable-kind and preview-plan-execution misuses under the same typed code, with no rejected value bound to it, and those classifications remain unreachable through the shipped command and are not report populations; strict index decoding X1–X10 keeps every `archive-index-*` classification; no message claims archive bytes are corrupt or present, recommends preserving or correcting index bytes, prints `rm`, or echoes a rejected raw selector value; and preview/confirmed read, mutation, lock and Git semantics are untouched. §6.4 places the new code in the **not evaluated** dry-run column, §9.7.2's preflight row and §10.4.1's catalog record it, and ADR-035 rev-20 carries the paired D16 vocabulary correction. D1–D21 are otherwise unchanged, the matrix stays at **567** rows with unchanged §18.52 arithmetic and **thirty-six** §18.53 semantic guards, and no row is added, retired, renumbered, re-categorised or re-kinded. Accepted on 2026-08-30, after the post-close external finding that raised it and **three** internal correction reviews of the drafted amendment, with the reviewed semantics, row set and arithmetic unchanged by acceptance. **Amended rows, exactly**: `PIB-431` and `PIB-465` — **two**. |
...
**APPROVED** with **no findings**, against writer tip `8f1cc8a` and errata tip
`0dd36e6` (WAVE_BASE `d060ff4`, [GH #11](https://github.com/tesseracode/tesserapatch/issues/11)).
The accepted contract is **567** acceptance rows (`PIB-001`…`PIB-567`), **176**
claims (`C1`…`C176`) and companion ADR-035 decisions **D1–D21**. Acceptance of
this PRD and of ADR-035 satisfies §19(1) and §19(2); the mutating slices remain
blocked **solely** on §19(3) — implementation and landing of the accepted
read-only `prepare --check` prerequisite (§17.1).
...
2. **Every X11 global storage inconsistency (§9.3.1).** A dangling retained
   ...
   **global** — it scans the whole index, not a selection (§9.7.1) — so an
   ...
   of state tpatch has already refused to trust. **§9.3.1's sequential
   admission does not reach this mode.** Admitting one repair class past the
   global scan is a property of the confirmed retention verb, which is
   repairing the archive; `--manual` is not repairing anything, so it refuses
   on every class it observes and renders every one of them with its own route
   (§9.3.1, PIB-538, PIB-552).
...
(created as **Proposed rev-12**, now **Accepted rev-14, 2026-08-14**),
keeps it in the ADR index, and requires it to be reviewed together with this
PRD. **Neither could be accepted alone**; both were accepted together at
rev-14 on 2026-08-14.
...
**The classification map below is total over a stated domain, and its precedence
is fixed.** A reference's disposition is decided from a **4-tuple**, and the
domain is spelled out rather than implied, because a table whose domain is not
stated cannot be checked for totality:

| Dimension | Values | Cardinality |
|---|---|---|
| **Wire state** — *this* reference's own recorded state | `retained`, `removal-pending`, `tombstoned` | 3 |
| **Blob observation** — the one observation the hash's file at `blobs/<h>.blob` supports | `absent`, `present, regular and hash-correct`, `present but non-regular or hash-wrong` (the **unidentifiable** observation) | 3 |
| **Ownership** — global over the hash: is any reference to `h`, in any generation, removal-pending? | `owned`, `not owned` | 2 |
| **Liveness** — global over the hash: is any reference to `h`, in any generation, retained **or** removal-pending? | `live`, `unreferenced` | 2 |

The Cartesian product is **36 tuples**. Ownership and liveness are separate
dimensions rather than one, because a hash can be live without being owned (a
retained reference and no pending one); they are not independent of the wire
state, and three dependencies — each a definition, not a heuristic — remove
exactly 18 tuples:

1. **A retained reference makes its own hash live.** Every retained row is
   therefore `live`; the 6 `retained × unreferenced` tuples are unreachable.
2. **A removal-pending reference makes its own hash both owned and live.** Every
   removal-pending row is therefore `owned, live`; the 9 remaining
   removal-pending tuples are unreachable.
3. **Owned implies live**, by (2) applied to the pending reference elsewhere in
   the index. For a tombstoned reference that removes the 3
   `tombstoned × owned × unreferenced` tuples.

6 + 9 + 3 = **18 unreachable**, leaving **18 reachable tuples** — 6 retained, 3
removal-pending and 9 tombstoned — and the table below has **exactly 18 rows,
one per reachable tuple, each with exactly one required next action**. PIB-551
derives both sides and fails a table with a missing row, a duplicate row, a row
over an unreachable tuple, or a tuple with two routes.

Precedence resolves the tuple in this order, so no tuple resolves to zero routes
or to two:

1. **Ownership first.** If `h` is owned, every reader but
   `feature intent-archive purge --yes` reports pending and routes to the owner,
   whatever the reference's own wire state or the blob's condition says. The
   owner itself resolves the tuple: with a present, regular, hash-correct blob
   it claims and completes; with an absent blob it publishes tombstones; and
   with an **unidentifiable** blob it refuses **exit 6
   `archive-purge-evidence-divergent`** with the evidence preserved. That is the
   **only** code an owned unidentifiable blob can produce — it is never
   `archive-index-storage-inconsistent` and never `archive-blob-corrupt`,
   because those are exit-3 zero-write classifications of a hash **no**
   transaction owns, and a hash whose removal is already half applied cannot be
   handed an exit-3 route that promises the tree was not written (§9.7.2 step 6,
   §10.4.1, PIB-558).
...
3. **Liveness last.** Only for a hash that is neither owned nor unidentifiable
   ...
   inconsistency when it is live, and a tombstoned reference beside an **absent**
   file is ordinary purged storage when the hash is unreferenced and the
   dangling classification (carried by the retained reference) when it is live
   (§9.3.1).

Every row below states its full tuple. The `retained` rows are `live` by
dependency (1) and the `removal-pending` rows are `owned, live` by dependency
(2), so those columns are stated once here rather than repeated per row.

| # | Wire state | Blob observation, ownership and liveness | Required next action |
|---:|---|---|---|
| 1 | retained | present, regular and hash-correct; `h` **not owned** (live) | ordinary live storage |
...
| 9 | removal-pending | present, **non-regular or hash-wrong** (owned, live) | refuse **exit 6** `archive-purge-evidence-divergent` and preserve evidence. This tuple and row 6 and row 12 are the same observation seen through three different references of one owned hash, and all three take that one code (§9.7.2 step 6, §10.4.1, PIB-558) |
| 10 | tombstoned | **absent**; `h` **not owned** and **unreferenced** | ordinary purged storage |
| 11 | tombstoned | **absent**; `h` **not owned** but **live** — some other reference to it is retained | not this reference's problem and not an inconsistency in it: the hash is a **dangling live reference**, classified through that retained reference at row 3, and this reference is already at the end state the confirmed `--blob <hash> --yes` repair will leave every reference to `h` in. It carries row 3's route and no second one (§9.3.1, §9.7.3) |
| 12 | tombstoned | **absent**; `h` **owned** (live) | the transaction in flight owns the hash; this reference is already at its end state and is nonetheless claimed and re-tombstoned with the rest, so the hash's wire state is uniform at every step (§9.7.2 steps 1 and 5) |
| 13 | tombstoned | present, regular and hash-correct; `h` **owned** (live) | the transaction in flight owns it; the recovery's global claim sweeps this reference into removal-pending and tombstones it again at the end. Every other command routes to the owner with `recovery-pending` (§9.7.2) |
...
file was hash-correct or unidentifiable, and one `tombstoned | absent, h not
owned` row that did not say whether the hash was live. The first collapse
mattered: rows 13 and 14 have **different exits** — 13 completes at exit 0 under
the owner and 14 refuses exit 6 — so a single row could be read as promising
that an owned tombstoned reference beside a directory or a hash-wrong file is
merely "swept into the claim", which is exactly the mis-mapping item 3 of the
rev-12 dispatch withdraws. The second mattered because a tombstoned reference
beside an absent blob is ordinary purged storage when the hash is unreferenced
and part of a **dangling** hash when it is live, and a reader of the collapsed
row had no way to tell which route applied. Both are now separate rows over
separate tuples, and PIB-551 derives the domain and the row count rather than
reading them (§9.3.1, PIB-551, PIB-558).
...
- **A global inconsistency refuses every *ordinary* mutation, zero-write.** An
  archive holding a mixed tombstone/live-reference hash `h₂` refuses every
  mutating `prepare` mode — `generate`, `manual` and `regenerate` — at exit 3
  with the whole tree byte-identical, naming `h₂`'s owning repair, whether or
  not that mode would have touched `h₂` (§6.2, PIB-538). It also refuses every
  confirmed purge selector that is **not** an admitted repair for a class the
  archive actually holds, on the same terms (§9.7.1, §9.7.2, PIB-542).
- **The scan always reports every class it found, whatever it admits.** The
  observation is over the whole index, so the report is over the whole index
  too: a refusal renders every observed class with its own route, and an
  *admitted* repair renders every class it did **not** touch with its own route
  and states that a rerun is required (PIB-541, PIB-548, PIB-556).
- **No partial cleanup proceeds *within* a class, and no unrelated class is
  half-acted-on.** A command never removes some of the orphans it could have
  removed, or purges some of the hashes of the class it is repairing, and then
  reports the rest as leftovers. Acting on part of a class that is lying about
  another part of itself is how a half-repaired archive becomes an archive
  nobody can reason about. What rev-12 does permit — and rev-11 did not — is
  completing **one whole class** while a **different** class remains, because
  that leaves an archive whose every remaining observation is exactly what the
  operator was just told to expect (see "Sequential multi-class repair" below).
- **The admitted repairs are scoped to the repair *class* they repair, and are
  admitted only after global classification.** §9.7.2's admissions are
...
- **A class may have many instances, and covering all of them is admitted.**
  ...
  no shipped repair at all. **Rev-11 replaced "sole inconsistency" with "sole
  ...
  same-class hash population (§9.7.2, PIB-548, PIB-549).
- **Rev-12 removes the last multiplicity brick: "sole class" becomes "one
  chosen class per invocation."** rev-11's rule still required the admitted
  class to be the **only** class the scan found, so an archive holding one
  unreferenced residue **and** one mixed hash had no admitted selector at all —
  `--orphans --yes` was withdrawn by the mixed hash and `--blob <h> --yes` by
  the residue — and every ordinary mutation refused too. That is the same brick
  rev-11 removed one level down, re-created between classes instead of between
  instances, and it is not hypothetical: §9.7.2's own compound ladder produces
  exactly that archive whenever a pending recovery finalizes beside a residue
  that a second, unrelated event created. **Sequential multi-class repair** is
  the fix, and it is stated as four conjunctive conditions below.
...
|---:|---|---|
| (a) | The confirmed selection **covers every instance of the chosen class** in the whole index | This is rev-11's coverage rule, unchanged. A selection covering part of its class is the intra-class partial cleanup that leaves an archive whose next observation differs from the one just acted on |
| (b) | **No selected hash, and no object the selection would remove, belongs to another class.** Class membership is decided by §9.3's precedence, so every hash belongs to exactly **one** class and the test is a set-disjointness check, not a judgement | This is what makes "one class" a property of the *mutation* rather than of the operator's intent. A selector that would sweep a second class's hash into its work list is repairing two classes at once under one confirmation, which is precisely what no confirmed selection may do silently |
...
`generate`, `manual` or `regenerate` — observing **any** class still refuses
exit 3 zero-write with the class's own route, because it is not repairing
anything: it would be publishing a lifecycle transition on top of an index that
is still lying about stored bytes (§6.2, §7.8, PIB-538).
...
cannot be shown total over a class in the general case, and admitting it would
require condition (b) to be re-derived per generation.

**`--all --yes` is admitted only under an explicit, separately stated
justification, and its blast radius is printed beside it every time it is
named.** `--all` selects **every retained reference in the archive** and is
allowed to remove **every** blob (§9.7.1), so it fails conditions (b) and (c)
against any second class by construction: it would sweep that class's hashes
into its own work list. It is therefore admitted **only** when the chosen class
is the **only** class the global scan found *and* the operator has been told, in
the same message, exactly what it costs. Wherever this PRD, ADR-035 or any
shipped string offers `--all --yes` as a repair, the offer carries all three of
the following, adjacent to the command and not in a footnote:

1. **What it does to the whole archive**: it claims and tombstones **every**
   reference in **every** generation and removes **every** blob — not only the
   class's instances — leaving an archive with no recoverable bytes for any
   artifact until identical content is archived again (§9.7.1, §9.7.3).
2. **Preview first, which is the default**: `purge <slug> --all` without `--yes`
   takes no lock, writes nothing and shows the full selection, and the message
   says so before the confirmed form appears.
3. **The narrower alternative**: repeated `--blob <h> --yes` covers the same
   class while touching nothing else, and is what the report recommends whenever
...
classes at once — most obviously a hash with a tombstoned and a retained
reference (mixed) whose blob is a directory (corrupt). §9.3's precedence decides
it once, globally, before any class set is built: ownership first, unidentifiable
bytes second, liveness last. So that hash is a `corrupt-object` instance and is
**not** in the `mixed-reference` class at all, which is why condition (b)'s
disjointness test is decidable: class membership is a function, not a
predicate that can hold twice. The consequence is stated so it cannot be
...
full global X11 scan, and it either refuses on `h₂` at exit 3 zero-write naming
`h₂`'s own repair or — when the rerun's selector is `h₂`'s class's admitted
repair and the four conditions above hold — repairs that class and reports every
class it left (§9.7.2). Ordering the two the other way would make an unrelated
...
This is fail-closed by choice — the **validation** is global and unconditional,
and only the **admission** is per class — and the alternative, validating only
the selected references, is recorded and rejected in §21, alongside rev-11's
withdrawn sole-class admission rule.
...
file is already gone (a removal spy records zero removals for that hash). The
selection is repeated once per dangling hash so that it covers the whole class
(§9.3.1's admission table). A confirmed `--all --yes` reaches the same end state
for the dangling hashes, but it is admitted **only** where `dangling-reference`
is the archive's sole class, and only where the message has stated in the same
breath that `--all` tombstones every reference in every generation and removes
every blob in the archive, that the unconfirmed `purge <slug> --all` preview is
the default and shows the full selection first, and that repeated
`--blob <h> --yes` covers the same class without touching anything else
(§9.3.1, PIB-557).
...
- **Why the removal step is `rm -rf --`, and why the message names no
  preservation command at all.** rev-10 printed `cp <managed blob path> <a path
  outside .tpatch/>`
  ...
  **directory** — one of the kinds PIB-543 fixtures — `cp` without `-R`
  ...
  includes the others. Rev-11 shipped **one type-total destructive
  step** and withdrew the copy promise; **rev-12 finishes the withdrawal** by
  removing the *named* alternatives from the prose as well:
  - `rm -rf -- <the validated repo-relative managed blob path>` removes a
    ...
    radius is exactly the object that refused. It is the **only** command this
    procedure emits.
  - **If the operator wants those bytes, they must stop here and preserve the
    object with tooling appropriate to its kind, chosen by them.** The message
    says exactly that and **names no command** — not `cp`, not `cp -R`, not
    `cp -P`, not `readlink`, not `git show`, not `mv`, not `rsync` and not
    `tar`. rev-11 withdrew the emitted `cp` and then listed those alternatives
    in prose, which reintroduced the same defect one layer out: a shipped
    remediation that names a kind-specific command is naming it for an object
    whose kind the operator has not been asked to confirm, and §10.7's permitted
    external forms do not include any of them. A wrong preservation command is
    worse than none — it either fails loudly after the operator has committed to
    the procedure, or succeeds while copying something other than the object
    they were told they were keeping — and a *list* of possibly-wrong commands
    is worse still, because it reads as a menu tpatch has vetted. PIB-559 is the
    token guard over every emitted block in this cluster (§10.7, PIB-547).
...
- **What repairs it, exactly.** One command, named literally, character for
  ...
  over `h` and over every other `mixed-reference` hash in the index, that is
  `--blob <h> --yes` repeated once per mixed hash. A confirmed `--all --yes`
  reaches the same end state, but only where `mixed-reference` is the archive's
  sole class and only with the whole-archive blast radius, the preview-first
  default and the narrower repeated-`--blob` alternative stated beside it
  (§9.3.1's admission table, PIB-557).
...
genuinely divergent evidence on an **owned** hash — a hash-wrong or non-regular
blob observed while some reference to that hash is removal-pending, whatever the
observing reference's own wire state — or an index that stopped strict-decoding
mid-run. Ownership is what puts it there: rev-11 said "under a pending
reference", which reads as a statement about *this* reference and left rows 6
and 14 of §9.3 — a **retained** and a **tombstoned** reference of an owned hash
beside an unidentifiable object — looking unclassified. They are the same
observation of the same owned hash and they take the same exit-6 code (§9.3,
§9.7.2 step 6, PIB-558). Neither tombstone-beside-blob
subcase is routed to §9.7.2's pending+absent escape, and neither is ever routed
to `--abandon-transaction`, and the same is true of the non-owned retained
corrupt-blob observation above (PIB-521, PIB-522, PIB-523, PIB-524, PIB-531,
PIB-532, PIB-533, PIB-535, PIB-543, PIB-558).

This keeps strict shape decoding while making every documented repair reachable
rather than permanently bricking the archive — including an archive holding more
than one repair class, which rev-12's sequential admission clears one class at a
time rather than refusing forever
...
- `--all` selects every retained reference and is allowed to remove every blob.
  ...
  tombstoned by the same state machine. **Its blast radius is the whole
  archive, and every offer of it as a repair says so.** Because it selects every
  retained reference, it fails §9.3.1's conditions (b) and (c) against any
  second repair class by construction — it would sweep that class's hashes into
  its own work list — so it is admitted as a class repair **only** where the
  chosen class is the archive's only class. Wherever it is named as a repair,
  the message states, adjacent to the command, that it tombstones every
  reference in every generation and removes every blob, that the unconfirmed
  `purge <slug> --all` preview is the default and shows the full selection
  first, and that repeated `--blob <h> --yes` covers the same class while
  touching nothing else (§9.3.1, PIB-557).
...
`--blob h` where `h` is well-formed but is carried by no reference in the
index, and `--generation id` where `id` is well-formed but is recorded by no
generation, both refuse exit 3 `archive-selector-invalid` with the whole tree
byte-identical, and never an `archive-index-*` code. Those two observations are
the whole public extent of the code. Selector **grammar** is not one of them: a
value that is not a full lowercase SHA-256, and a missing or repeated scope
family, never reach a report at all — the command's own scope check and
selector normalization reject them at exit 1 before any archive is opened,
printing nothing, carrying no refusal code and echoing no rejected value
(§9.7.2, PIB-347). The distinction is load-bearing: the two exit-3
observations say only that the **selection** matched nothing, and an
archive that is absent, empty or perfectly healthy produces them just as
readily as one that is not, so a refusal claiming index corruption there would
send an operator to preserve and repair bytes that may not exist. The refusal
therefore states which selector named nothing, and routes to
`feature intent-archive list <slug>` run from the workspace root, whose output
either names a value to rerun with or shows that this archive holds none
(§9.7.2, §10.4.1, PIB-431, PIB-465).
...
| **The file at `blobs/h.blob` is replaced between step 2's revalidation and step 3's unlink** | **Refused, with the replacement preserved — except inside the probe→unlink gap.** Step 3 is not a bare unlink of the path step 2 read: the removal carries the identity step 2 recorded and the shipped storage re-probes the managed object immediately before the syscall, refusing to remove anything whose identity no longer matches. For a regular blob that identity is the recorded mode, size and content hash, so only a byte-identical object passes the re-probe and nothing distinct is lost when it does. A replacement that lands at that point therefore stops the transaction rather than being deleted: exit **5** `archive-purge-partial` on branch (a) with `resume: pending-recovery-then-completion` and `pending_hash: h`, zero completed hashes, the replacement byte-identical on disk, every reference to `h` still removal-pending, and the sanitized rerun refusing exit **6** `archive-purge-evidence-divergent` over the managed blob path with its own repair procedure (§9.7.2 step 6, §10.4.1). **What stays open is the gap between that re-probe and the unlink syscall**, because no primitive on either supported platform removes a path only if its contents still hash to a value observed a moment earlier, and there is no seam inside it. **The probe→unlink gap is not detected, and is not claimed to be.** It is the same disclosed **final-syscall residual** the canonical publication carries (§7.1, §7.8), narrowed here to one syscall gap and stated so it is not discovered later as a silent loss. Its consequence is bounded: the hash is being purged, so the index's end state (every reference tombstoned, no blob) is truthful either way; what an external writer can still lose in that gap is bytes it placed there after tpatch had already decided to remove that path. No shipped message claims this window is closed, and PIB-550 fails any that does |
...
lands in an existing classification with an existing repair. **Two of them keep
a disclosed byte-level residual** — the probe→unlink gap inside the
revalidate→unlink window above, which the removal's identity re-probe narrows
but does not close, and the post-CAS rename window below — and both are stated
as residuals rather than resolved, because the lock excludes tpatch mutators
only (PIB-540, PIB-544, PIB-550).
...
|---|---|
| selector present, exactly one, and well-formed (`^[0-9a-f]{64}$` hashes, known generation ids) | exit 1 (no selector, a repeated scope family, or a value that is not a full lowercase SHA-256 — all three are rejected by the command's own scope check and selector normalization before any archive is read, with an empty report, no refusal code and no echo of the rejected value) / exit 3 (`archive-selector-invalid` for a well-formed `--blob` hash the index does not carry, and for a well-formed `--generation` id the index does not record). The exit-3 half is the **selector's** own refusal and never an archive-integrity claim: it says the selection named nothing, states no observation about the archive's bytes, and routes to `feature intent-archive list <slug>` from the workspace root so the operator can choose a value the archive actually holds — or stop, when it holds none, which is exactly the state of a feature that has no archive at all. No `archive-index-*` code is reachable from it. The store keeps its own defensive classification of the arity and malformed-value populations under the same typed code, binding no rejected value to it, but that classification is unreachable through the shipped command and is never a report population (§9.7.1, §10.4.1, PIB-347, PIB-431, PIB-465) |
...
retained `h`, a confirmed global `--blob h --yes` — repeated once per dangling
hash so the class is covered, or `--all --yes` where that class is the archive's
only one and the blast radius has been stated —
...
3. **The rerun** then reaches the preflight with no pending hash left, performs
   ...
   **class**: an instance of the same class the selector does not cover, a
...
**This population is exactly the owned-hash and pending-index evidence, and
...
reference of a **non-owned** hash is not pending, so there is no pending hash to
name, no purge
...
reference of a **non-owned** hash beside a present but non-regular or hash-wrong
blob is outside this population for the same reason — no reference to that hash
is pending — and is exit-3
`archive-blob-corrupt` with §9.3.1's type-total removal-then-confirmed-purge or
restore route (PIB-543).

**Ownership, not this reference's wire state, is what puts an unidentifiable
blob in this population.** rev-11 wrote the boundary as "under a **pending**
reference", which is true of the observation but reads as a test on the
observing reference. It is not: the population is every hash the purge
transaction **owns** — any reference to it removal-pending — whose blob is
present but non-regular or hash-wrong, observed through *any* of that hash's
references. §9.3's rows 6, 9 and 14 are one hash seen through a retained, a
pending and a tombstoned reference, and all three take **exit 6
`archive-purge-evidence-divergent`**. rev-11's residual exit-3
`archive-index-storage-inconsistent` mapping for "an unsafe/wrong pending blob"
— carried in the X11 cell, in §10.4.1 and in ADR-035 D10 — is **withdrawn**: it
contradicted §9.3 and this step 6, and it offered a zero-write exit-3 promise
for a transaction whose first mutation had already landed, which exit 3 cannot
honour (§9.3, §10.4.1, ADR-035 D10, PIB-558). None of the four non-owned classes is in this
population, and PIB-524 fails any classification that puts either
tombstone-beside-blob subcase back
(PIB-506, PIB-508, PIB-558).
...
**Blob divergence** — the hash is **owned** (some reference to it is
removal-pending) and its blob is present but hash-wrong, or is a non-regular
file. The managed blob path is the divergent object:
...
feature intent-archive purge <slug>: refused archive-purge-evidence-divergent
  ...
  matches what it is. tpatch does not name one, because no single command is
  correct for every kind of object that can be at that path.
...
**Why one destructive command, and why the message names no preservation
command either.** This population's fixtures
...
refusal, so rev-11 printed exactly one removal — `rm -rf --` against the single
validated repo-relative managed path, with the `--` terminator, no wildcard and
no recursion beyond that path — and told the operator plainly to stop and use
kind-appropriate tooling if they want the object preserved. **Rev-12 removes the
named alternatives from that sentence too**: the emitted block above names no
`cp`, `cp -R`, `cp -P`, `readlink`, `git show`, `mv`, `rsync` or `tar`, in the
procedure or in the prose beside it, because a shipped remediation that names a
kind-specific preservation command is vouching for it against an object whose
kind the operator has not confirmed, and §10.7's permitted external forms
contain none of them. The destructive
warning is printed **above** the command, not after it (§9.3.1, §10.7,
PIB-506, PIB-507, PIB-547, PIB-559).
...
object at a managed blob path in the gap between the removal's identity re-probe
and the unlink, which the unlink itself cannot condition on; a replacement that
lands before that re-probe is refused with the object intact (§9.7.2's window
table). Both residuals are the same disclosed
...
PIB-521…PIB-524, PIB-527, PIB-528, PIB-531…PIB-535, PIB-539…PIB-560).
...
hashing to its own name: on a **non-owned** hash such an object is a
`corrupt-object` instance whatever that hash's liveness, it is never an orphan,
and while one exists no confirmed selector runs at all (§9.3.1's repair-class
precedence, PIB-561, PIB-562). On an **owned** hash — one carrying a
removal-pending reference — the same object is not a `corrupt-object` instance
and not an orphan either: ownership outranks, and the owning `purge --yes`
refuses **exit 6** `archive-purge-evidence-divergent` (§9.3, §9.7.2 step 6,
PIB-558). List,
...
archive-divergence procedure of §9.7.2 is never named for it, because no
reference to that hash is pending for that procedure to finalize (§9.3.1,
PIB-521, PIB-522, PIB-523, PIB-524).
...
run from the workspace root, repeated once per mixed hash so that the class is
covered. It selects **every** reference to that hash, claims
...
(§9.3.1 subcase B, PIB-531, PIB-532, PIB-533, PIB-535). A confirmed
`--all --yes` reaches the same end state, but it is offered only where
`mixed-reference` is the archive's only class, and only with the sentence that
it tombstones every reference in every generation and removes every blob, that
the unconfirmed `purge <slug> --all` preview is the default and shows the full
selection first, and that repeated `--blob <h> --yes` covers the same class
without touching anything else (§9.3.1, PIB-557). If instead any
...
a **retained** reference of a **non-owned** hash is `archive-blob-corrupt`
(exit 3), rendered `corrupt`
...
**The worked example below is that exit-3 archive-integrity refusal**, because
its plan's `stages[0]` is still the rank-1 `corrupt-object`
`manual-prerequisite`: while any `corrupt-object` instance is present, every
confirmed selector refuses zero-write (§9.3.1, PIB-561), so no invocation over
this archive can have repaired a class. The report therefore carries
`outcome: "refused"`, `action: "none"` and **no** `repaired_class`. The exit-0
admitted form is the same object with `repaired_class` added and no
`manual-prerequisite` stage left in `stages[]`:
...
Every field is normative, including `repaired_class`, which the example above
omits because it is a refusal's plan:

- `rerun_required` is the constant `true`. The object exists only to say that
  the archive is **not** consistent yet, so a consumer switching on the field
...
- `repaired_class` is the closed class token this invocation was admitted for,
  ...
  is a refusal's plan", which is exactly what the exit code already says. It
  can never appear beside a `manual-prerequisite` stage, because a plan that
  still carries the rank-1 `corrupt-object` prerequisite is by §9.3.1's own
  precondition a plan every confirmed selector was refused against.
...
### 10.3 Advisory catalog (closed, eighteen codes)
...
artifact row. PIB-227 asserts the shipped catalog is exactly these eighteen
...
| `archive-selector-invalid` | 3 | **the selection is at fault, and the archive is not accused of anything.** The code has exactly two emitted populations, both on `feature intent-archive purge`: a well-formed lowercase 64-hex `--blob` hash the index carries no reference to, and a well-formed `--generation` id the index records no generation for. Both exit **3**, zero-write, in the preview and the confirmed form alike. Selector **grammar** is not one of them: a missing or multiple scope family, and a `--blob` or `--generation` value that is not a full lowercase SHA-256, are usage errors that the shipped command's own scope check and selector normalization reject at exit **1** before any archive read — no report is printed, no refusal code is carried, and a rejected malformed value is never echoed (§9.7.1, PIB-347). The message names which selector matched nothing and carries the offending value, which in both emitted populations is an already-validated lowercase hash or generation id. The remediation is the listing route — the literal `tpatch feature intent-archive list <slug>` run from the workspace root, then a rerun with a value that listing names, or a stop when it names none. It never says archive bytes are corrupt, absent or present, never asks for preservation, correction or restoration of index or blob bytes, names no `rm` form, and carries no `retry`/`retry_cwd` pair, because the next command is an inspection the operator must read rather than a repair to paste. `archive-index-*` codes remain reserved for X1–X10 strict decoding and X11 storage observation and are unreachable from a selector fault. The store additionally classifies the arity, malformed-value, unusable-kind and preview-plan-execution misuses under the same typed code as defence in depth; none of them is reachable through the shipped command, so none of them is a report population (§9.7.1, §9.7.2, §10.7, PIB-431, PIB-465) |
...
| `archive-purge-evidence-divergent` | 6 | preserve every blob, the index and the pending record, and name the **archive** procedure of §9.7.2 — the pending hash, the repo-relative managed blob and `index.json` paths, an explicit destructive warning, the type-total `rm -rf --` of the divergent managed blob path (blob form) **or** the restoration of an index that no longer strict-decodes (index form, which names no removal at all), and the sanitized rerun with `retry_cwd`, whose pending+absent recovery terminally tombstones — together with its destructive cost and the Git-history caveat. **No preservation command is named**, because no one copy command is correct for a regular file, a symlink, a directory, a FIFO and a device node alike and a list of them reads as a vetted menu (PIB-547, PIB-559). It **must not** name `--abandon-transaction`: that mode touches no index and no blob and cannot consume this evidence (§6.6 rules 8 and 10, PIB-506, PIB-508). Its population is every hash a purge transaction **owns** — some reference to it removal-pending — whose blob is present but non-regular or hash-wrong, observed through **any** of that hash's references, plus the pending-index form; §9.3's rows 6, 9 and 14 are one owned hash seen through a retained, a pending and a tombstoned reference and all three take this code, which is the **only** code reachable for them (PIB-558). A **non-owned** tombstoned reference beside a present blob is instead exit-3 `archive-index-storage-inconsistent`, with the `--orphans --yes` repair when the hash is globally unreferenced and the confirmed `--blob <h> --yes` repair when it is still live (§9.3.1, PIB-521, PIB-524, PIB-531), and a **non-owned** retained reference beside a present but unidentifiable blob is exit-3 `archive-blob-corrupt` with §9.3.1's own route (PIB-543) |
...
(§9.7.2). `archive-selector-invalid` is `feature intent-archive purge` only —
no other command takes a purge selector — and it is reachable only from that
command's **preflight**, on a well-formed selector value the archive does not
carry, before any archive mutation and independently of whether the feature has
an archive at all. The catalog
...
22. Archive/index/storage integrity (§9.2, §9.3.1 X1–X11), shared-selection
    ...
    selector does not touch refuses every **ordinary** mutation zero-write and
    names that hash's own repair; no partial cleanup of unrelated items
    proceeds around it (PIB-542). **A confirmed purge selector may be admitted
    for exactly one repair class under §9.3.1's four conditions**: it covers
    every instance of that class, no selected hash or removed object belongs to
    another class, the mutation cannot erase or degrade another class's
    evidence, and the report names every untouched class with its own route and
    states that a rerun is required. A second class merely being present no
    longer withdraws the admission — that was rev-11's brick — but an uncovered
    same-class instance, a selection reaching into a second class, or a
    mutation that would degrade another class's evidence still refuses
    zero-write and names each class's own route (PIB-548, PIB-549, PIB-552,
    PIB-555, PIB-556). A tombstoned reference whose named blob is present, on a
    hash **no** transaction owns, refuses
    ...
    `--blob <h> --yes` repeated once per mixed hash; each is the invocation
    admitted past that
    observation, admitted only for that observation's class, and `--orphans` never
    removes a live blob (§9.3.1, PIB-521,
    PIB-522, PIB-531, PIB-532, PIB-542). A **retained** reference of a
    **non-owned** hash whose blob is
    ...
    An **owned** hash is not classified at this step at all: it was routed or
    resolved before it, and where its blob is unidentifiable the owning
    `purge --yes` refuses **exit 6** `archive-purge-evidence-divergent` rather
    than any exit-3 code (§9.3, §9.7.2 step 6, PIB-558).
    For the
...
**Form (1) is scoped to §9.5's success report and may not appear anywhere
else.** It restores an archived artifact the operator asked to restore; it is
**not** a preservation aid, and it is forbidden in every corrupt-object,
divergent-evidence and repair-class message. Mixing the two contexts is how a
`cp` line ended up beside a destructive step in rev-10 (PIB-559).

**Form (3) is destructive, type-total and deliberately unaccompanied by any
preservation command.** rev-10 permitted a `cp`/`rm` *pair* here and rev-11
withdrew the
...
kind **and names no command** — rev-11 removed the emitted `cp` and then listed
`cp -R`, `cp -P`/`readlink` and `git show` in the prose beside it, which is the
same defect one layer out, since none of those three is a permitted form and
naming them vouches for them against an object whose kind the operator has not
confirmed.
...
| D6 | `doctor` gains check `D9` (§12.5) — evidence-only, zero writes, and it never opens or flocks the workspace root. The recovered-panic finding message is a constant for **every** doctor check, so D1–D8 no longer interpolate the panic value either — a safety hardening rather than a behavior delta, and their goldens are unchanged because no D1–D8 golden fixture drives a panicking check. | `doctor` |
...
| `docs/adrs/README.md` | The ADR-035 index row (created with this PRD at rev-0; update its proposed rev-14 status when implementation is authorized). |
...
| R24 | A `purge --orphans --yes` removes the residue it was asked about while a mixed tombstone/live-reference hash elsewhere in the same index goes **unmentioned**, leaving an operator who believes the archive is repaired and discovers otherwise on their next ordinary mutation. | X11's storage observation is over the **whole index** before every archive mutation, independent of the selector (§9.3.1), and the report is over the whole index too. Since rev-12 the residue **is** repaired in that archive — refusing it was rev-11's brick — but only under §9.3.1's four conditions, of which the fourth is that the report names every untouched class with its own literal repair and states that a rerun is required, carried by the `archive-repairs-remaining` advisory and the closed `remaining_repairs` object (§10.2, §10.3). What remains forbidden is a **partial** repair *within* a class, a selection that reaches into a second class, and a silent omission. PIB-542 asserts the refusal where no admission applies, PIB-552 and PIB-556 assert the admitted run and its untouched-class report, and `list`/`doctor` render every observation at once so one pass shows the whole inventory (PIB-541). |
| R25 | An operator finds a present but hash-wrong blob under a retained reference, gets a refusal with no route, and either edits the index by hand or deletes the archive. | The observation is `archive-blob-corrupt`, zero-write, and pinned across `list`, `doctor` and every ordinary mutation: `list` and every ordinary mutation refuse it at exit **3**, and `doctor`'s D9 renders the identical observation as a **warning** finding and exits **0**, which is ADR-035 D16's warning-only rule. All of them carry one repo-relative procedure: one type-total `rm -rf --` of the managed blob path under an explicit destructive warning, then the confirmed `purge <slug> --blob <h> --yes` — which by then is the ordinary, already-admitted dangling repair — or restore the exact correct blob and retry. The cost and the Git-history caveat are stated. PIB-543 asserts the classification, the route and the end-to-end composition, and PIB-547 asserts the printed command executes on all five object kinds (§9.3.1, §9.7.3, §10.7). |
...
| R29 | An operator's archive accumulates **two different** repair classes — say an unreferenced residue and a mixed hash — and rev-11's "sole class" admission withdraws every selector, so the archive has no shipped repair at all and every ordinary mutation refuses forever. | Admission is **sequential**, one chosen class per invocation, under §9.3.1's four conditions: full coverage of the chosen class, disjointness from every other class's hashes and objects, provable non-degradation of every other class's evidence, and a report naming every untouched class with its own route and requiring a rerun. The global scan still observes and reports **every** class, and every ordinary mutation still refuses. PIB-552 drives the two-class sequence end to end, PIB-553 the three-class sequence including the corrupt class's manual prerequisite, PIB-555 derives the non-degradation predicate from the implementation, and PIB-549 fails an admission predicate that keeps the sole-class rule (§9.3.1, §9.7.1, §9.7.2). |
| R30 | A hash satisfies two class descriptions at once — a mixed tombstone/live pair whose blob is a directory — and a confirmed `--blob <h> --yes`, admitted as the mixed class's repair, silently acts on an object the corrupt class forbids tpatch to touch. | Class membership is a **function**, not a predicate that can hold twice: §9.3's precedence — ownership first, unidentifiable bytes second, liveness last — collapses overlapping observations on one hash to exactly one class before any class set is built, so that hash is `corrupt-object` and is not in the mixed class at all. Its selector is therefore **not** admitted until the manual prerequisite has run. PIB-554 derives the collapse from the classifier and fails an implementation that lets one hash appear in two class sets or that admits a selector for the lower-precedence description (§9.3, §9.3.1). |
...
| R38 | An operator purges with a hash or generation id the archive does not carry — a typo, a stale value from an old report, or any selector at all against a feature that has no archive — and the refusal tells them their archive index is corrupt and to preserve its bytes, so they go looking for damage that does not exist, or repair an archive that is not there. | The selection and the archive are separately classified: a well-formed `--blob` hash or `--generation` id the index does not carry is `archive-selector-invalid` (§10.4.1), which states only that the selection named nothing and routes to `feature intent-archive list <slug>` from the workspace root. A malformed value or a missing/repeated scope family reaches no report at all and stays an exit-1 usage error, so it can not carry a false accusation either (§9.7.1, PIB-347). `archive-index-*` codes stay reserved for X1–X10 strict decoding and X11 storage observation, no selector refusal claims bytes are corrupt or present, none asks for preservation and none prints an `rm` form. PIB-431 fails the typed archive refusal renderer if it relabels an already-public catalog code — the exact shape of the shipped defect — and PIB-465 asserts both selector families over an absent as well as a populated archive. |
...
was implemented and accepted by GH #16 before GH #23 dispatched. The shipped
inspector, slug grammar, state enum, report routing and
`internal/cli/prepare.go` are the baseline every slice below extends. Three
rules follow and remain hard:
...
2. **Its goldens must be produced by its accepted implementation, not by a
   ...
   **pre-production golden commit**, drives record mode with a clean binary
   whose embedded `vcs.revision` is exactly GH #16's frozen implementation
   content `cacaaf8` (production-identical to formal acceptance commit
   `7206dab`) and whose `vcs.modified` is false, and makes normal comparison
   mode build current code while refusing a supplied baseline binary. A
   fixture recorded from the GH #23 baseline/current binary fails before it
   can be written. The committed fixture does not contain a cryptographic
   producer attestation; permanent re-audit rebuilds `cacaaf8` and repeats the
   guarded recording procedure.
...
rather than a fault. **rev-12 adds exactly one test-only seam, and it is not a
fault-injection point**: PIB-560's injected **file-kind** seam, which reports an
observed object's kind to the classifier so the device-node case can be
exercised where `mknod` needs privilege the test user lacks. It has no
production call site, it is used only for that one kind, and the substitution is
recorded as a stated limitation in the test's own output. Every other rev-12
state — the multi-class archives, the class-collapse hash, the owned-corrupt
tuples, the split state-map rows and the forbidden-token blocks — is an index or
filesystem fixture, a control-flow derivation or a printed string.
...
**Rev-12 explicitly amends**, rather than silently re-meaning, every stable row
whose observable moved with this revision — and only those rows, each one
...
- *Printed-command parity and the forbidden-token set* (§9.3.1, §9.7.2, §9.7.3,
  §10.7): `PIB-506`, `PIB-543`, `PIB-547`.
- *State-map domain and the two row splits* (§9.3): `PIB-524`, `PIB-545`,
  `PIB-551`.
- *Real non-regular fixtures and the injected device-kind seam* (§9.3.1,
...
existing entries. Their old sole-class-admission,
pending-blob-is-exit-3, `cp`-named-in-prose, collapsed-owned-tombstone-row and
...
- *`--all` emitter totality* (§9.7, §9.7.1, §9.7.3, §10.2): `PIB-528`,
  `PIB-557`.
- *Tokenized command guard and the normative “triple”→“tuple” residue* (§9.3,
  §10.7): `PIB-547`, `PIB-559`. `PIB-524` is **not** an amended row: rev-13
  touches it only inside its §18.53 semantic-fixture entry, so its amendment is
  **fixture-only** and its acceptance-matrix row is unchanged in the rev-13
  diff.
...
fixtures inside their existing entries, and `PIB-524`'s existing entry is
amended in wording only — the normative “triple”→“tuple” correction — which is
the whole of rev-13's touch on that ID. Their old
...
**Rev-14 is errata only**, and amends exactly two rows — derived from the rev-14
diff against `8f1cc8a`, not from this prose: `PIB-565` (its “twelfth” `outcome`
token becomes **thirteenth**, since rev-13's own `purged` made the closed set
twelve) and `PIB-567` (its “corrected throughout” fixture is qualified to
**normative** occurrences, and rev-13's own `PIB-524` mis-claim joins its
driving fixtures). Each is amended in its §18.53 semantic-fixture entry with the
same wording. No row is added, retired, renumbered or re-kinded; the matrix
stays at **567**, §18.52's arithmetic is untouched, and §18.53 stays at
**thirty-six** guards.

**Rev-15 is an implementation-evidence erratum only**, and amends exactly one
stable row: `PIB-391`. Its impossible path-history predicate is replaced by
the clean accepted-producer binding and current-binary comparison rule stated
in §17.1; the matching §18.53 sensitivity entry changes from “right bytes at a
wrong past path commit” to wrong-producer, self-comparison and late-fixture
...
**Rev-17 is an implementation-discovered, no-decision erratum only** — and it
is **accepted**, jointly with rev-18 and rev-19 on 2026-08-29 — and it amends
exactly three stable rows: `PIB-542`, `PIB-543` and `PIB-544`. `PIB-542`'s first
fixture asserted that all four selectors refuse exit 3 over an archive holding
one repair class (the mixed hash `h₂`) beside a healthy `h₃`. That contradicts
the frozen rev-12 admission rule and ADR-035 D16: `--all --yes` is admitted
when the chosen class is the archive's only class, and a healthy hash is in
**no** class, so it makes no second class. The corrected row keeps the three
no-class selectors refusing zero-write and states the admitted `--all --yes`
outcome with its mandatory whole-archive blast-radius disclosure. The clean
control, the two-mixed-hash fixture and the two-class residue-plus-mixed
fixture are unchanged. `PIB-543` required exit 3 from **every** observer and
one procedure presentation from all five; the product's `doctor` D9 is
warning-only and exits **0** — which D16 already states — while `prepare`,
`--regenerate`, `--manual` and `list` exit 3, and the same removal + retry
route is printed in two presentations: `list` carries the archive's quoted
procedure with the structured `retry`/`retry_cwd: "workspace-root"` pair,
`doctor` carries the same quoted procedure lines without that pair, and the
three mutating readers carry the route as refusal `remediation` prose over the
unquoted repo-relative managed path. Every D16 obligation on those
procedures — repo-relative paths only, the destructive cost, the Git-history
caveat, the single exact-path `rm -rf --`, no preservation command, no
`--orphans`, no forbidden command word — is unchanged and still asserted on all
five. `PIB-544` named `beforePurgeIndexCAS`, `afterPurgeBlobRevalidate`,
`beforePurgeBlobRemove` and `beforePendingTombstoneCAS` as the four window
drivers; the authoritative mapping is the one the lower store fixture
`TestRecoverPendingPurgeInsertionWindowsPIB544` already drives — the
post-preflight index capture, `beforePurgeIndexCAS`, `afterPurgeIndexRename`
and the post-unlink `beforePendingTombstoneCAS` — with `afterPurgeBlobRevalidate`
excluded because it is PIB-550's fifth-window seam. No window's exit, outcome,
resume token, removal truth or retry behaviour changes. None of the three rows
has a §18.53 semantic-fixture entry, so none is touched. No row is added,
retired, renumbered, re-categorised or re-kinded; the matrix remains **567**,
§18.52's category/kind/slice arithmetic is unchanged, and §18.53 remains at
**thirty-six** semantic guards.

**Rev-18 is an implementation-discovered, no-decision erratum only** — and it
is **accepted**, jointly with rev-17 and rev-19 on 2026-08-29 — and it amends
exactly one stable row: `PIB-550`. That row, and the fourth row of §9.7.2's window
table it tests, said an external replacement of the managed object landing
between step 2's revalidation and step 3's unlink is removed and that the
byte-level loss is undetected. The shipped machine hands step 2's revalidated
identity to the removal, and `prepareArchiveStorage.RemoveBlob` re-probes the
managed object before the syscall and refuses when that identity no longer
matches, so the replacement is preserved and the invocation ends at exit **5**
`archive-purge-partial`, with the sanitized rerun at exit **6**
`archive-purge-evidence-divergent`. The corrected row and window row state that
outcome and narrow the disclosed residual to the unseamed gap between the
re-probe and the `root.Remove` syscall, which is still neither detected nor
claimed closed. §9.7.2's closing residual paragraph, §9.7.2's exit-6 atomicity
note and §21's corresponding alternatives row are amended to the same truth;
ADR-035 rev-18 carries the paired D16 correction. `PIB-550` is kinded `C` and
has no §18.53 semantic-fixture entry of its own; the review that raised this
erratum also required two guards to grow fixtures, so the §18.53 entries of
`PIB-546` and `PIB-551` gain the fixtures the implementation added — the shared
per-hash helper reached by an unscanned ordinary route, and the row-13/row-14
split asserted through the purge machine's own exit classes rather than through
the row count. Neither entry's row, kind, category or count changes. No row is added,
retired, renumbered, re-categorised or re-kinded; the matrix remains **567**,
§18.52's category/kind/slice arithmetic is unchanged, and §18.53 remains at
**thirty-six** semantic guards.

**Rev-19 is a review-discovered, no-decision erratum only** — and it is
**accepted**, jointly with rev-17 and rev-18 on 2026-08-29 — and it
amends exactly two stable rows: `PIB-562` and `PIB-566`.
Those two are the only acceptance-matrix rows it touches, in either direction.
`PIB-562`'s closing sentence stated one exit for `list` and `doctor` jointly —
they “render the corrupt object and both residues in one pass, exit 3”.
`doctor`'s D9 is warning-only, which ADR-035 D16 states and the accepted rev-17
`PIB-543` correction restates, and the shipped exit mapper returns **0** for a
run that raises only warnings. The corrected row keeps `list` rendering every
observation in one pass at exit **3** and states separately that `doctor`
renders that identical observation set as **warning** findings at exit **0**;
the two-stage/one-invocation totals, the empty `resulting_classes`, the
byte-identical index and the ban on naming `--orphans --yes` as the corrupt
object's repair are unchanged. `PIB-566`'s derived emitter population omitted
§12.5's `doctor` D9 pending-purge remediation, which observes the same
removal-pending reference set and prints the same narrow lexically sorted
command, and the row required `retry_cwd: "workspace-root"` from **every**
emitter although `doctor` carries that command as remediation prose and has no
`retry`/`retry_cwd` pair of its own. The corrected row adds the `doctor`
emitter and scopes `retry_cwd` to the structured retry carriers, keeping the
no-`--all`, exact-pending-set, no-inherited-`--path` and narrow-versus-`--all`
recovery-equivalence obligations on every relevant carrier.

Three further corrections in this erratum are **companions, not matrix rows**,
and are named here so the ledger above stays at exactly two. §10.2's worked
sequential-repair example is rewritten from an unreachable exit-0 `purged`
report — one carrying `repaired_class` beside a plan whose `stages[0]` is still
the rank-1 `corrupt-object` `manual-prerequisite` — into the exit-3
archive-integrity refusal form the same paragraph already describes, keeping
the corrupt-object manual stage and dropping `repaired_class`, which the
standing rule already says is present if and only if the invocation repaired a
class. §16's `R25` is split by surface exactly as `PIB-562` is. `PIB-566`'s
§18.53 semantic-fixture wording scopes its omitted-`retry_cwd` fixture to a
structured retry carrier, and §18.53 still lists **three** fixtures for that
guard. ADR-035 rev-19 carries the paired D10 prose correction and changes no
D10 or D16 decision. No row is added, retired, renumbered, re-categorised or
re-kinded; the matrix remains **567**, §18.52's category/kind/slice arithmetic
is unchanged, and §18.53 remains at **thirty-six** semantic guards.

**Rev-20 is a bounded public diagnostic-vocabulary amendment, not a no-decision
erratum** — it was **accepted** on 2026-08-30, after the post-close external
...
It is not an erratum because the closed public code a purge selector that names
nothing emits **changes**, and §10.4.1's catalog grows from **53** to **54**
codes. The defect was symmetric: `internal/store` classified a `--blob` value
the index does not carry as `archive-selector-invalid`, the CLI refusal builder
rewrote that code to `archive-index-corrupt` because the catalog did not
contain it, and the store's own unknown-generation branch emitted
`archive-index-corrupt` directly — so a feature with no archive and a
well-formed unindexed hash was told its index was corrupt and to preserve
archive bytes that did not exist. rev-20 surfaces the **existing** code rather
than inventing a second name, scopes it to the two populations that actually
reach a report — a well-formed `--blob` hash the index carries no reference to,
and a well-formed `--generation` id it records no generation for — and gives it
the selector-specific message and listing remediation of
§10.4.1. `PIB-431` is the narrowest existing row that owns refusal-code
rendering across the catalog, human and JSON surfaces, so it gains the
renderer-scoped no-relabel obligation and the selector-specific rendered pair;
`PIB-465` is the narrowest existing row that owns the §9.7.2 preflight
**selector** row for both the `--blob` and the `--generation` family, so it
gains the emitted code, the preserved typed value, the absent-archive
population, the exit-1 non-report half and the unchanged
`archive-index-*` classification of its strict-decode sibling. No third row
owns either behaviour, and `PIB-347` is deliberately **not** amended: the
missing/multiple-selector grammar it owns keeps its exit-1 parse refusal
unchanged, and so does the malformed-value normalization beside it — neither
becomes a catalog population. The companion §6.4 dry-run partition, §9.7.1,
§9.7.2's preflight row, §10.4.1, §16's `R38`, §21's alternatives rows and
ADR-035 rev-20's D16
vocabulary correction are **companions, not matrix rows**, and are named here
so the ledger above stays at exactly two. Neither amended row has a §18.53
semantic-fixture entry, so none is touched. No row is added, retired,
renumbered, re-categorised or re-kinded; the matrix remains **567**, §18.52's
category/kind/slice arithmetic is unchanged, and §18.53 remains at
**thirty-six** semantic guards.
...
| PIB-391 | G | the `--check` golden fixtures used by PIB-198 … PIB-207 | the fixtures' first Git appearance precedes every GH #23 mutating production path; record mode requires a clean producer binary with embedded `vcs.revision == cacaaf8` (GH #16 frozen implementation content, production-identical to formal acceptance `7206dab`) and refuses the GH #23 baseline/current binary, while comparison mode always builds current code and refuses every supplied baseline override; permanent producer re-audit rebuilds `cacaaf8` and repeats record mode |
...
| PIB-431 | G | refusal catalog, precedence, human and JSON renderers | every reachable code has one exit and identical code/remediation across catalog/human/JSON; purge/publication index codes stay distinct; **the typed archive refusal renderer surfaces an already-public catalog code unchanged** — `intentArchiveRefusalFromError` may not relabel one catalog code as another, so `archive-selector-invalid` reaches the report as itself and never as an `archive-index-*` code, and that rewrite reintroduced in that renderer fails the same validator. The obligation is scoped to that renderer and claims nothing about boundaries the validator does not read: an internal, non-catalog transport failure such as `archive-storage-failed` is legitimately classified into a catalog code by its owning boundary, which is what `prepareStoreArchiveFailure` does, and that mapping stays permitted. Its rendered message and remediation are the selector-specific pair of §10.4.1 — which selector named nothing, the `feature intent-archive list <slug>` route from the workspace root, and no corruption, preservation or `rm` wording — identical in human and JSON |
...
| PIB-465 | C | one fixture per §9.7.2 preflight row (selector, strict decode, whole-index X11 storage, shared reference, orphan identity, reference count) | each exits 3 with a whole-tree byte snapshot identical to the pre-run tree; no pending record, tombstone or blob removal occurred. The **selector** row is asserted over both of its emitted populations and both selector families: a well-formed value naming no indexed content hash and a well-formed value naming no recorded generation each refuse `archive-selector-invalid` — never an `archive-index-*` code — with the typed hash or generation id preserved on the classified error, over an absent/empty archive as well as a populated healthy one. Its exit-1 half is asserted separately and as a **non**-report population. The missing/repeated scope-family check runs before workspace discovery or authority. In an otherwise admissible workspace with no pending journal, a malformed `--blob`/`--generation` value exits 1 in preview and confirmed forms with empty stdout, no refusal code, no echo of the rejected value, no archive mutation and no Git; preview takes no authority, while confirmed takes and releases exactly one workspace authority before normalization. Workspace, platform, pending-journal and confirmed contention refusals retain their higher precedence and are not reclassified as selector usage errors. The store's own defensive classification of malformed values binds no rejected value to the typed error. The strict-decode row keeps its `archive-index-*` classification unchanged. The X11 fixture is run with a selector that is **disjoint** from the inconsistent hash as well as with one that is not, and both refuse identically (PIB-542) |
...
| ID | Kind | Case | Asserted observable |
|---|---|---|---|
| PIB-506 | I | the `archive-purge-evidence-divergent` refusal, in each of its sub-populations — a hash-wrong regular file under a **pending** reference; a symlink, a directory, a FIFO and a device node where a **pending** blob belongs; and an index that stopped strict-decoding mid-run | exit 6; the message names the pending hash and the repo-relative `blobs/<hash>.blob` and `index.json` paths, prints the explicit destructive warning **above** the command, names the single type-total `rm -rf -- <managed blob path>` (blob forms) or the restoration step and **no** removal at all (index form), names the sanitized rerun with `retry_cwd: "workspace-root"`, and states both the destructive cost and the Git-history caveat; no `cp`, `git`, `readlink`, `mv`, `rsync`, `tar`, `ln`, `install`, `dd` or `chmod` command word and no wildcard appears in any of them, in the procedure or in the prose beside it (PIB-559); the string `--abandon-transaction` appears **nowhere** in it, and no absolute path appears anywhere in it; the population is scoped by **ownership**, not by the observing reference's wire state: the same unidentifiable object under a **retained** and under a **tombstoned** reference of the same owned hash produces this same exit-6 code (§9.3 rows 6 and 14, PIB-558), while a tombstoned reference whose named blob is present on a hash **no** transaction owns produces exit **3** `archive-index-storage-inconsistent` instead and never this code, in **both** its globally-unreferenced and its still-live subcase (PIB-521, PIB-531) |
| PIB-507 | I | executing the documented blob-form procedure after PIB-506, once per object kind — run the printed `rm -rf --` verbatim against the managed blob path, then the sanitized rerun | the printed command is executed **for real** against a **real filesystem fixture** for the four kinds any unprivileged process can create — regular file, symlink, directory and FIFO — succeeding with a shell exit of 0, removing exactly that path, following no symlink to its target and touching no sibling; the **device-node** kind runs the identical assertions against the injected file-kind seam of PIB-560 where `mknod` requires privilege the test user lacks, and the substitution is recorded in the test's own skip/limitation output rather than presented as a real-filesystem result. In every kind the rerun then takes the absent-blob branch of §9.7.2, exits 0 with `outcome: "recovered"`, tombstones every reference to that hash, removes nothing, and leaves an index that strict-decodes; a subsequent ordinary archive mutation proceeds (PIB-547, PIB-560) |
...
| PIB-542 | I | `feature intent-archive purge <slug> --yes` on **four disjoint selectors** — `--orphans`, `--blob <h₃>`, `--generation <id₃>` and `--all` — over an archive whose *unrelated* hash `h₂` is a mixed tombstone/live-reference inconsistency, where `h₃`/`id₃` provably touch no reference to `h₂` and **no** unreferenced residue exists, so `--orphans`, `--blob <h₃>` and `--generation <id₃>` are no class's admitted repair, while `--all` covers every instance of the mixed class — the archive's **only** class, since the healthy `h₃` belongs to no class and therefore makes no second one | the **three no-class selectors** exit **3** `archive-index-storage-inconsistent` with a whole-tree byte snapshot identical to the pre-run tree: nothing is removed, `h₃` is **not** purged, no pending record is written and `index.json` is byte-identical; each message names `h₂`'s literal `--blob <h₂> --yes` repair rather than the selector the operator typed; running that repair and then re-running the original selector completes at exit 0. `--all --yes` is instead **admitted** past the same whole-index refusal, because the mixed class is the archive's only class and `--all` covers every instance of it: it prints, adjacent to the command, the mandatory whole-archive blast-radius disclosure — every reference in every generation tombstoned and every blob removed — with the preview-first default and the narrower repeated-`--blob <h₂> --yes` alternative (PIB-557), then purges the whole archive and exits **0** `purged` with `action: "none"`, leaving no class outstanding and `h₃`'s blob removed along with the rest, exactly as a confirmed whole-archive purge promises. A decode spy confirms the whole-index X11 scan ran before each of the three refusals **and** before the admission, and a second fixture in which `h₂` is clean shows all four selectors succeeding, so the refusal of the three is attributable to `h₂` alone. A third fixture holds **two** mixed hashes and shows that `--blob <h₂> --blob <h₂'> --yes` — the class fully covered — is admitted and repairs both, while `--blob <h₂> --yes` alone is refused zero-write for leaving an instance of its own class uncovered. A fourth fixture adds a removable unreferenced residue beside `h₂` and shows the rev-12 boundary directly: `--orphans --yes` is now **admitted** for the residue class and exits 0 reporting `h₂` untouched with its route (PIB-552), while `--blob <h₃> --yes`, `--generation <id₃> --yes` and `--all --yes` still refuse exit 3 zero-write — the first two because they are no class's repair, `--all` because it would reach into `h₂`'s class (§9.3.1, §9.7.1, §9.7.2, PIB-548, PIB-549, PIB-552, PIB-555, PIB-557) |
| PIB-543 | I | a **retained** reference of a **non-owned** hash whose `blobs/<h>.blob` is present but (i) a symlink, (ii) a directory, (iii) a regular file whose bytes hash to something else, (iv) a FIFO and (v) a device node — the first four built on the real filesystem and the fifth through PIB-560's injected file-kind seam — observed by an ordinary mutating `prepare`, by `--regenerate`, by `--manual`, by `list` and by `doctor`, then repaired | the four refusing observers — `prepare`, `--regenerate`, `--manual` and `list` — exit **3** `archive-blob-corrupt`, and `doctor` reports the identical corrupt-object observation as a **warning** finding and exits **0**, which is D16's warning-only D9 rather than an exception to it; every one of the five leaves a whole-tree byte snapshot identical to the pre-run tree, and `list` renders the storage state `corrupt`. All five name the same repo-relative route, in the product's two presentations of it. `list` renders the archive procedure on the affected entry — the explicit destructive warning, the single POSIX-quoted `rm -rf -- '<managed blob path>'` — and carries the structured `retry` `tpatch feature intent-archive purge <slug> --blob <h> --yes` with `retry_cwd: "workspace-root"`; `doctor`'s D9 remediation renders the same quoted warning and removal lines followed by the `After every removal above, run <that same literal retry>` line; and `prepare`, `--regenerate` and `--manual` render the route as the refusal's `remediation` prose — the destructive warning, the unquoted `rm -rf -- <managed blob path>` and that same literal retry — carrying no `retry`/`retry_cwd` pair of their own. Wherever a surface prints the retry as a human command line it sits under the one column-0 retry heading, which is PIB-498's totality obligation, not a per-surface JSON field. All five carry the restore-the-correct-blob alternative, the destructive cost and the Git-history caveat, with no absolute path anywhere, no `--orphans` mentioned and none of the forbidden command words `cp`, `git`, `readlink`, `mv`, `rsync`, `tar`, `ln`, `install`, `dd` or `chmod` appearing in the procedure or in the prose beside it (PIB-559). A sixth fixture makes the same hash **owned** by adding a removal-pending reference and asserts the observation moves entirely to exit-6 `archive-purge-evidence-divergent`, with this code emitted nowhere (PIB-558). Executing the procedure completes on **all five** object kinds: after the removal the reference is dangling, the confirmed purge is admitted past X11, tombstones every reference to `h`, performs zero removals and exits 0, and a following ordinary mutation proceeds. Writing the correct blob back instead also clears it with nothing purged (§9.3.1, §9.7.3, §10.7, PIB-547) |
| PIB-544 | C | an external same-hash reference inserted into `index.json` at each of the four insertion windows of §9.7.2 — before the claim's re-read, between the re-read and its CAS, between the CAS and the pre-removal revalidation, and after the removal — driven through the seams the authoritative store fixture `TestRecoverPendingPurgeInsertionWindowsPIB544` uses: the post-preflight index **capture** (store `before:capture-index` on its second capture; at CLI level a `CaptureIndex` spy armed once `PreflightIndexCAS` has run), then `beforePurgeIndexCAS` (store `before-index-cas`), then `afterPurgeIndexRename` (store `after-index-cas`, immediately after the claim's CAS publication), then the post-unlink `beforePendingTombstoneCAS` (store `after-remove`). `afterPurgeBlobRevalidate` is deliberately **not** a driver here — it fires after the pre-removal revalidation, which is the fifth window, external **replacement** of the managed object between revalidation and unlink, owned by PIB-550 — and `beforePurgeBlobRemove` observes the removal syscall rather than opening a window of its own | window 1 completes normally with the inserted reference claimed and tombstoned like every other; windows 2 and 3 perform **no** removal and end in `archive-purge-partial` at exit 5 with `resume: "pending-recovery-then-completion"` and `pending_hash: h` — the pending record already existed, so the post-first-mutation window is open — and the reported sanitized retry re-derives the claim and completes at exit 0; window 4 is detected by the tombstone CAS and its retry takes the absent-blob path, tombstoning the inserted reference with the rest. A fifth fixture inserts the reference against a **new** selection before its first write and asserts exit 3 `archive-purge-index-changed` with a whole-tree byte snapshot. In no window is a blob removed while a non-pending reference to it is visible in the index the command validated, and in no window does the command loop in-process (§9.7.2, §7.12) |
| PIB-545 | G | the archive's storage-classification, **claim** and removal authority, **derived** from the implementation rather than from a table: the map over (wire state × blob observation × the hash's global **ownership** × its global **liveness**), the domain of the claim CAS, and the guarded precondition of every blob removal | every reachable tuple resolves to exactly one disposition and one route: any tuple whose hash is **owned** resolves to `recovery-pending` for every reader except `purge --yes`, which claims it; retained/absent → `archive-blob-dangling`; retained/present-unsafe-or-wrong and **not owned** → `archive-blob-corrupt` with §9.3.1's type-total procedure; **any owned hash with an unsafe-or-wrong blob, through a retained, a pending or a tombstoned reference alike → `archive-purge-evidence-divergent`, exit 6, with no exit-3 code reachable** (PIB-558); tombstoned/present-regular-hash-correct with the hash unreferenced → `--orphans --yes`; tombstoned/present-regular-hash-correct with the hash live-but-not-owned → the confirmed `--blob <h> --yes`; tombstoned/present-unsafe-or-wrong and **not owned** → `archive-blob-corrupt`, the corrupt classification dominating both; accepting tuples unchanged. The guard additionally asserts, by control-flow walk rather than name scan, that (a) the claim CAS's reference set is **every** reference with the target hash — retained and tombstoned alike, with no wire-state predicate excluding any of them — and (b) **every** blob-removal call site is dominated by a check that no reference to that hash is anything but removal-pending. Seven semantic sensitivity fixtures, each spelled correctly and formatted validly, must fail: one that classifies an owned hash as mixed or dangling instead of routing it; one that removes a blob under a hash with a surviving retained reference; one that maps retained/present-hash-wrong on a non-owned hash to `archive-blob-dangling`; one that admits `--orphans --yes` to *remove* a mixed hash's blob; one that maps an **owned** unsafe-or-wrong blob to an exit-3 code instead of exit-6 `archive-purge-evidence-divergent`; **one whose claim CAS skips already-tombstoned references while the removal gate still requires every reference to be pending — the deadlock rev-10 shipped in prose**; and one whose absent-blob path tombstones only the pending references and leaves a retained one behind |
...
| PIB-550 | C | the revalidate→unlink window: an external writer replaces the object at `blobs/<h>.blob` between the pre-removal revalidation and the unlink, injected at `beforePurgeBlobRemove` after `afterPurgeBlobRevalidate` has passed, over both a replacement regular file and a replacement directory | the replacement is refused rather than removed, because the removal re-probes the managed object's identity before the syscall: both kinds end at exit **5** `archive-purge-partial` with `resume: pending-recovery-then-completion`, `pending_hash: h`, zero completed hashes, the replacement byte-identical on disk and every reference to `h` still removal-pending, and each sanitized rerun refuses exit **6** `archive-purge-evidence-divergent` naming the managed blob path. The guard half asserts the disclosure rather than the behavior: §9.7.2 and ADR-035 each disclose the residual probe→unlink syscall gap **and** the post-CAS final-syscall race, in that order, each anchor occurring exactly once per document; no shipped string and no sentence of either document claims this window is closed, conditioned or wholly detected; and one-delta sensitivity fixtures that add "the removal is conditioned on the revalidated content" to either document or to a report, that drop the post-CAS race, or that restore this window row's retired undetected-removal reading must each fail (§7.1, §9.7.2) |
...
### 18.50 AW — Rev-12 adjudication: sequential multi-class repair, printed-command parity, the owned-corrupt route and the state domain
...
| PIB-391 golden provenance | a recorder supplied the GH #23 baseline binary with the right output bytes, a comparison run supplied the accepted baseline binary so it compared the fixture with itself, or a fixture's first Git appearance followed an already-landed mutating production path; all three must fail before comparison/recording or acceptance |
...
| PIB-546 recovery-exception ordering | a control flow that runs the whole-index X11 scan before `RecoverPendingPurge`, so a pending hash cannot be finalized while an unrelated hash is inconsistent and the refusing command is the only repair the refusal names; a recovery that continues into the selector after finalizing; and a mutating `prepare` granted the same exception instead of refusing `recovery-pending`. Rev-18 adds a fourth: an ordinary selector execution hoisted ahead of the plan and a `ExecuteIntentArchivePurge` that trusts the planned selection, so the ordinary route reaches the **same per-hash helper the recovery reaches** with no scan before it — which a walk that memoised that helper by name and scan-state alone would accept on a cache hit, reporting the recovery's route only. All are valid code with correct names |
| PIB-549 sequential class admission | an admission predicate that keeps rev-10's "sole inconsistency" rule, so an archive holding two unreferenced residues is refused under every selector and has no shipped repair at all; one that keeps rev-11's "sole class" rule, so an archive holding one residue and one mixed hash is refused for the same reason one level up; one that admits a selection covering only part of its class and repairs it partially; one that admits `--all --yes` while a second class is present, whose whole-archive write set erases that class's evidence; and one that admits a selection for a hash whose §9.3 precedence puts it in a **different** class than the description the selection matched |
| PIB-551 wire/storage disposition totality | a table that drops the retained/absent-and-**owned** tuple, so an owned dangling reference is classified `archive-blob-dangling` instead of routed to its transaction; one that gives the tombstoned/present-unidentifiable tuple two routes by offering both `--orphans --yes` and the corrupt procedure; one that re-collapses rows 13 and 14 into rev-11's single `tombstoned \| present, and h is owned` row, so an owned tombstoned reference beside a directory is described as merely swept into the claim; and one whose row count and stated domain disagree — 17 or 19 rows over the same 18 reachable tuples — which a count-free totality check would pass. Rev-18 adds four that bite the split's actual route rather than its row count: a table that keeps all eighteen rows but gives row 14 row 13's owner-sweep wording, one that gives row 13 row 14's **exit 6** `archive-purge-evidence-divergent` route, and two source demotions of the purge machine the guard derives that route from — the shared owned arm demoted to the not-owned exit-3 classification, and the per-hash evidence check demoted to exit 3 while the owned recovery preflight still refuses exit 6 |
...
| PIB-565 pinned purge outcome | a successful purge emitting `outcome: "published"`, which a harness switching on the field cannot distinguish from a canonical publication; a thirteenth `outcome` token reachable only from the purge path; and a `repair_cwd` spelled `workspace_root` |
| PIB-566 pending-route narrowing | an emitter that widens to `--all --yes` whenever more than one hash is pending — rev-12's own wording, a real shipped command, and unbounded where the exact pending set was in hand; one that names only the first pending hash, leaving the rest owned after the operator's rerun; and one that names the right hashes on a structured retry carrier but omits that carrier's `retry_cwd` |
| PIB-567 revision-ledger parity | a ledger listing an amended row the revision's diff did not change — rev-12's own `PIB-548` claim; one omitting a row the diff did change; and a “corrected throughout outside quoted/meta references” claim whose scan still finds the corrected token in normative prose |
...
**The gate is satisfied and closed.** Conditions (1)–(4) are complete: rev-15's
...
3. ✅ **The accepted `prepare --check` PRD must be implemented, landed on
   `origin/main`, and passing its own acceptance matrix** — §17.1. —
   **Satisfied by GH #16 before WAVE_BASE `3b579fc`, independently of
   rev-15.** Rev-15 governs the additional compatibility fixtures consumed by
   this implementation wave, not whether GH #16 landed.
4. ✅ The cluster lead declares the §17.2 file partition and dispatches,
   ...
   2026-08-18; rev-15 was jointly approved the same day, and the mutating
   production slices S1–S7 landed and were accepted through aggregate
   review.**

The sequencing block is closed. Rev-15 was not a product-design reopening: it
corrected one impossible evidence predicate discovered while implementing the
mandatory golden-first step, and once jointly approved the §17.2 production
order continued unchanged to completion.
...
| Claiming the revalidate→unlink window is closed by re-checking the file's hash immediately before removal | **Rejected as unachievable by a re-check, and therefore not claimed — though the re-check itself ships.** The removal does re-probe the managed object's identity immediately before the syscall and refuses a replacement it catches, which is what makes that case an exit-5 refusal with the object preserved rather than a silent deletion. It cannot close the window: neither supported platform offers an unlink conditioned on content, so a replacement landing in the gap between that re-probe and the syscall is still removed. The remaining gap is disclosed as a residual beside the post-CAS rename race rather than closed in prose, and PIB-550 fails any shipped string or document sentence that says otherwise (§9.7.2, §7.1). |
...
| Admitting a repair only when its class is the **sole** class in the index (rev-11) | **Withdrawn as the same brick one level up.** rev-11 correctly replaced rev-10's per-instance rule with a per-class one, and then required the class to be the only class present. An archive holding one unreferenced residue and one mixed hash therefore had no admitted selector at all — `--orphans --yes` was withdrawn by the mixed hash and `--blob <h> --yes` by the residue — while every ordinary mutation refused, which is exactly the unrepairable state the class rule was introduced to remove. Admission is now **sequential**: one chosen class per invocation, under four conjunctive conditions (full coverage of the class, disjointness from every other class, provable non-degradation of every other class's evidence, and a report naming every untouched class with its route and requiring a rerun). The global scan is unchanged — it still validates and reports the whole index, and every ordinary mutation still refuses zero-write (§9.3.1, §9.7.1, §9.7.2, PIB-552, PIB-553, PIB-555, PIB-556). |
| Letting a confirmed selector repair **two** classes in one invocation | **Rejected.** The operator confirmed one repair, and the classes have different blast radii, different evidence and different costs — a residue removal touches nothing referenced, while a mixed-hash purge destroys recoverable bytes. Collapsing them under one `--yes` would make the confirmation cover a decision the operator was never shown. One class per invocation, with the next class's own literal command reported, keeps every destructive step individually confirmed (§9.3.1, PIB-552, PIB-556). |
| Admitting `--all --yes` as a class repair while a second class is present | **Rejected as a whole-archive mutation dressed as a class repair.** `--all` selects every retained reference in the archive, so its write set necessarily includes the other class's hashes: it fails the disjointness condition and the non-degradation condition together, and would erase the very evidence the untouched-class report is supposed to hand the operator. It is admitted only where the chosen class is the archive's only class, and every offer of it states the whole-archive blast radius, the preview-first default and the narrower repeated-`--blob` alternative (§9.3.1, §9.7.1, PIB-555, PIB-557). |
| Mapping an unsafe or hash-wrong blob on an **owned** hash to exit-3 `archive-index-storage-inconsistent` (rev-11) | **Withdrawn as a contradiction and a false promise.** rev-11's X11 cell, §10.4.1 and ADR-035 D10 carried that exit-3 mapping while §9.3's table and §9.7.2 step 6 mapped the same tuple to exit-6 `archive-purge-evidence-divergent`. Exit 3 means the tree was not written; a hash the purge transaction owns has already had its first mutation land, so exit 3 cannot be honoured for it, and the exit-3 route names repairs that are defined over non-owned hashes. Every observation of an owned hash with an unidentifiable blob — through a retained, a pending or a tombstoned reference — is exit 6 with §9.7.2's archive procedure, and no exit-3 code is reachable for it (§9.3, §9.3.1, §10.4.1, PIB-558). |
...
| Keeping the CLI rewrite of `archive-selector-invalid` into `archive-index-corrupt` (shipped through rev-19) | **Withdrawn as a false public statement.** The rewrite existed only because §10.4.1's catalog was closed without the code the store already emitted, and it made the diagnostic say the opposite of what the classifier observed: a selection that matched nothing was reported as a corrupt index, with a preservation remediation, on features whose archive is absent. A closed catalog is a reason to add the missing code, never a reason to relabel a classification (§10.4.1, PIB-431). |
| Inventing a second, purge-specific code name for the unknown-id population | **Rejected.** `archive-selector-invalid` already exists in the shipped classifier and already means exactly *the selection is unusable*. A second name would split one population across two public codes, force every consumer to learn both, and grow the closed catalog by two rather than one (§9.7.1, §10.4.1). |
| Giving the selector refusal a structured `retry`/`retry_cwd` pair | **Rejected for this population.** The next step is `feature intent-archive list <slug>`, an **inspection whose output the operator must read** before choosing a value — and when the listing names nothing, there is no next command at all. A structured retry field is the shape reserved for a command that can be pasted unread; naming the listing in the remediation keeps the route honest without promising a repair (§10.4.1, §10.7). |
| Promoting the malformed-value and missing/multiple-selector populations to the same exit-3 structured refusal (rev-20 as first drafted) | **Rejected as a behaviour change this amendment is not entitled to make.** The shipped command rejects both before any archive is read — the scope check for the family count, the selector normalization for the value shape — so both exit **1** with empty stdout, no refusal code and no echo of the rejected value, in the preview and the confirmed form alike, and the pinned CLI contract asserts exactly that. Surfacing them as catalog refusals would move an exit code and an emitted report shape, which a bounded diagnostic-vocabulary amendment may not do, and it would make the catalog promise a population no operator can reach. The store keeps its defensive classification of them, unreachable and unemitted (§9.7.1, §9.7.2, PIB-347, PIB-465). |
...
±5 lines of the cited construct at dispatch/base `a2a6479` (reviewed tip
`8f1cc8a`). Claims corrected in rev-1
...
rev-10, rev-11, rev-12, rev-13 or rev-14
