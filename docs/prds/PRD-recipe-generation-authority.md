...
does not consume either decision. §6.4 now cites ADR-024 D1 explicitly for the
...
every field, vocabulary and reason code. It applies ADR-024 D1's separate
per-feature manifest boundary
(`docs/adrs/ADR-024-patch-generation-manifest-boundary.md:48-64`) without
folding coverage into `status.json` or `patch-generations.json`. This section
reproduces the canonical block byte-identically and adds nothing:
