...
### Post-acceptance citation erratum — 2026-09-02

Removed ADR-010 and ADR-025 from **Depends on** because this producer contract
does not consume either decision. D3 now cites ADR-024 D1 explicitly for the
...
Consistent with ADR-024 D1's separate per-feature manifest boundary
(`docs/adrs/ADR-024-patch-generation-manifest-boundary.md:48-64`), coverage is
a strict sidecar rather than a field on `apply-recipe.json`, `status.json` or
an entry in `patch-generations.json`. It has independent schema and lifecycle
authority rather than extending patch-generation history.
