...
with rev-18 and rev-19 (raised 2026-08-27); S7 AU implementation discovery
corrected **three** companion acceptance-matrix rows (PRD `PIB-542`, `PIB-543`,
...
corrected D16's residual disclosure and the companion row `PIB-550`. Both said
the replacement of a managed object landing between the pre-removal revalidation
...
that **D10's** corrupt-blob paragraph states one refusal for `list`, `doctor` and
...
slices S1–S7). The sequencing prerequisite is satisfied by GH #16, and the
...
**Its implementation is a hard prerequisite, not a companion.** GH #16
satisfied that prerequisite before GH #23 dispatched. GH #16 committed its
executable AVP suite but no standalone ready/not-ready/abort output files, so
path history cannot prove the producer of those later-frozen bytes. The
compatibility fixtures instead come from a clean binary whose embedded
revision is GH #16 frozen implementation content `cacaaf8` (production-identical
to formal acceptance commit `7206dab`); record mode refuses another producer,
and comparison mode always builds current code while refusing a supplied
baseline. The fixture commit precedes every mutating production edit (PRD
§17.1, PIB-391).
