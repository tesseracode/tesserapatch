...
**Consumer note — fast-path JSON returns less than `status.json` (GH #5 follow-up):** when `confirm-upstreamed <slug>` takes the v0.11 "fast path" (feature already `upstreamed`/`confirmed-upstreamed` on entry), its `--json`/`--format json` payload emits `upstream_commit` and `upstream_ref` as **empty strings** — the populated values already recorded in `status.json` from a prior confirmation are NOT propagated through the fast-path JSON, to preserve byte-identical output across releases (PRD-confirm-upstreamed-human-review-path §AC-2). Only the human-review path's JSON payload carries the real `upstream_commit`/`upstream_ref` values (plus `source_revision_entry_id`/`transition_revision_entry_id`). Consumers that need the full record regardless of which path fired should read `tpatch status --json <slug>` instead of depending on `confirm-upstreamed`'s fast-path JSON shape.
