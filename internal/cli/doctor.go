...
func doctorCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Diagnose tpatch workspace metadata drift.\n\n" +
			"Checks feature metadata (D1), patch-generations manifests (D2), installed skill assets at the six init-managed paths (D3), the current required lock file .tpatch/upstream.lock (D4), reconcile evidence/revision JSONL artifacts (D5), recipe schemas (D7), and workspace invariants (D8).\n" +
	...
	cmd.Flags().StringArrayVar(&checks, "check", nil, "Limit execution to a doctor check ID (repeatable: D1, D2, D3, D4, D5, D7, D8)")
	return cmd
}
