...
func doctorCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Diagnose tpatch workspace metadata drift.\n\n" +
			...
			"D10 checks recipe generation coverage and independent capture evidence. It is warning-only and never writes or fixes artifacts, even under --fix; regeneration guidance requires a read-only plan of the actual record command.\n" +
	...
	cmd.Flags().StringArrayVar(&checks, "check", nil, "Limit execution to a doctor check ID (repeatable: D1, D2, D3, D4, D5, D6, D7, D8, D9, D10)")
