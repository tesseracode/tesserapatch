...
func TestValidateDoltArgs(t *testing.T) {
	cases := []struct {
		...
		{"valid", doltArgs(nil), "users", "", 0},
		{"missing-contract", doltArgs(map[string]string{"contract": "\x00DELETE"}), "users", ReasonInvalidDeclaration, ExitValidation},
		{"unsupported-contract", doltArgs(map[string]string{"contract": "dolt-diff-summary-v2"}), "users", ReasonDoltContractUnsupported, ExitValidation},
		{"missing-db-path", doltArgs(map[string]string{"db_path": "\x00DELETE"}), "users", ReasonInvalidDeclaration, ExitValidation},
		{"missing-table", doltArgs(map[string]string{"table": "\x00DELETE"}), "users", ReasonInvalidDeclaration, ExitValidation},
		{"missing-from", doltArgs(map[string]string{"from": "\x00DELETE"}), "users", ReasonInvalidDeclaration, ExitValidation},
		{"missing-to", doltArgs(map[string]string{"to": "\x00DELETE"}), "users", ReasonInvalidDeclaration, ExitValidation},
		{"unknown-key", append(doltArgs(nil), store.ResourceArg{Key: "extra", Value: "x"}), "users", ReasonInvalidDeclaration, ExitValidation},
		{"duplicate-key", append(doltArgs(nil), store.ResourceArg{Key: "table", Value: "users"}), "users", ReasonInvalidDeclaration, ExitValidation},
		{"control-byte", doltArgs(map[string]string{"from": "ma\x01in"}), "users", ReasonDoltArgumentRefused, ExitValidation},
		{"nul-byte", doltArgs(map[string]string{"table": "us\x00ers"}), "us\x00ers", ReasonDoltArgumentRefused, ExitValidation},
		{"backslash", doltArgs(map[string]string{"to": `HEAD\x`}), "users", ReasonDoltArgumentRefused, ExitValidation},
		{"dot-range", doltArgs(map[string]string{"from": "main..HEAD"}), "users", ReasonDoltArgumentRefused, ExitValidation},
		{"working-upper", doltArgs(map[string]string{"from": "WORKING"}), "users", ReasonDoltArgumentRefused, ExitValidation},
		{"working-mixed", doltArgs(map[string]string{"from": "Working"}), "users", ReasonDoltArgumentRefused, ExitValidation},
		{"staged-lower", doltArgs(map[string]string{"to": "staged"}), "users", ReasonDoltArgumentRefused, ExitValidation},
		{"selector-table-mismatch", doltArgs(nil), "orders", ReasonInvalidDeclaration, ExitValidation},
	}
...
func TestParseDiffSummaryJSON(t *testing.T) {
	...
	cases := []struct {
		...
		{"zero-rows-bare", `{}`, 0, false},
		{"zero-rows-real-shape", "{}\n\n", 0, false},
		{"nonempty-real-shape", `{"rows":[` + row + "]}\n", 1, false},
		{"nonempty-no-trailing-whitespace", `{"rows":[` + row + `]}`, 1, false},
		{"leading-whitespace", "  \t{\"rows\":[" + row + "]}\n", 1, false},
		{"empty-rows-array", `{"rows":[]}`, 0, false},
		{"schema-key-refused", `{"schema":[],"rows":[]}`, 0, true},
		{"extra-top-level-key", `{"rows":[],"extra":1}`, 0, true},
		{"rows-not-array", `{"rows":{}}`, 0, true},
		{"missing-rows-key", `{"data":[]}`, 0, true},
		{"not-json", `not json`, 0, true},
		{"not-an-object", `[]`, 0, true},
		{"row-missing-field", `{"rows":[{"from_table_name":"u","to_table_name":"u","diff_type":"modified","data_change":true}]}`, 0, true},
		{"row-extra-field", `{"rows":[{"from_table_name":"u","to_table_name":"u","diff_type":"modified","data_change":true,"schema_change":false,"x":1}]}`, 0, true},
		{"row-duplicate-key", `{"rows":[{"from_table_name":"u","from_table_name":"u","to_table_name":"u","diff_type":"m","data_change":true,"schema_change":false}]}`, 0, true},
		{"boolean-as-int", `{"rows":[{"from_table_name":"u","to_table_name":"u","diff_type":"modified","data_change":1,"schema_change":0}]}`, 0, true},
		{"boolean-as-string", `{"rows":[{"from_table_name":"u","to_table_name":"u","diff_type":"modified","data_change":"true","schema_change":"false"}]}`, 0, true},
		{"name-as-number", `{"rows":[{"from_table_name":1,"to_table_name":"u","diff_type":"modified","data_change":true,"schema_change":false}]}`, 0, true},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
