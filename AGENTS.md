...
- [ ] **`.wave-close-allowlist` growth reviewed.** If this wave added entries to `.wave-close-allowlist` (used by `[2/8]` to silence known-untracked WIP files), each new entry has an operator disposition tracked in `docs/handoff/CURRENT.md` or `docs/handoff/HISTORY.md` — not just a bare path addition. Unbounded allowlist growth without a tracked disposition reverts Cluster E-prime Obs 2's fix: the sentinel becomes noise again.
...
**Mechanical gate**: `make wave-close-check` runs 8 programmatic checks (working tree clean, untracked source-code sentinel, HEAD pushed, trailer parses, CURRENT.md Status line not stale, gofmt clean, vet/build clean, CI-equivalent uncached test shards clean). Human items (LOG prepended, ROADMAP flipped, HISTORY archived, non-invalidation invariants) remain manual — the target prints them as a reminder banner. Codified 2026-08-02 after Cluster A external challenge #2 flagged that the checklist above is protocol-only with no verifier. **Test-suite check added 2026-08-04 (Cluster E F1)**: prior to this, the gate reported PASS while `go test -count=1 ./...` exited 1 at Cluster D HEAD `1bc2a25` — correctness was the one dimension the gate didn't check. Every shard uses `-count=1` to disable the build cache, so the check is deterministic even when source is unchanged between gate invocations. **CI-equivalent sharding added 2026-08-29**: GH #23's accepted suite made the unsharded `internal/cli` process exceed Go's 10-minute default under the repository's required serial resource discipline. `[8/8]` invokes `scripts/wave-close-test-shards.sh`, which pins `BASH_ENV=/dev/null`, empty `GOFLAGS`, `GOENV=off`, `GOMAXPROCS=1` and `-p=1`, then runs the exact blocking-CI test partition in 22 fresh processes with the same per-process 40-minute timeout; `prepare_s7_ci_timeout_guard_test.go` enforces command and environment parity. **`.wave-close-allowlist` added 2026-08-05 (Cluster E-prime Obs 2)**: `[2/8]` reads this repo-root file (one glob/literal path per line, `#` comments) and subtracts matches from the untracked-source WARN list, since the same 16 known-WIP files WARNed on every gate run across Clusters C/D/E, training reviewers to skim past the sentinel. Residual (non-allowlisted) untracked files still WARN exactly as before.
