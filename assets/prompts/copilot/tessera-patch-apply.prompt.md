...
`tpatch init`, `tpatch add`, `tpatch status`, `tpatch analyze`, `tpatch define`, `tpatch explore`, `tpatch implement`, `tpatch apply`, `tpatch record`, `tpatch land`, `tpatch reconcile`, `tpatch reconcile confirm-upstreamed`, `tpatch reconcile audit-retirement`, `tpatch provider check`, `tpatch config show|set`, `tpatch cycle`, `tpatch test`, `tpatch verify` (integrity/freshness checks, including `recipe_generation_coverage`), `tpatch feature patch refresh`, `tpatch feature patch fixup`, `tpatch doctor` (including read-only D10 recipe coverage), `tpatch next`, `tpatch session start`, `tpatch session stop`, `tpatch session list`, `tpatch session summarize`, `tpatch session purge`
...
Only a real producer repairs the pair, never a reader inventing evidence.
